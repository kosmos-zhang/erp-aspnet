...
{
   public  class InputPersonTrueIncomeTaxBus
    {
        public static DataTable SearchPersonTaxInfo()
        {
            //获取登陆用户信息
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            //执行查询
           
            return InputPersonTrueIncomeTaxDBHelper.SearchPersonTaxInfo(userInfo.CompanyCD);
        }
        ...
        public static bool SaveInsuPersonInfo(IList<PersonTrueIncomeTaxModel> modelist)
        {
            ...
            if (modelist != null && modelist.Count > 0)
            {
                ...
                {
                    //执行保存操作
                    isSucc = InputPersonTrueIncomeTaxDBHelper.UpdateIsuPersonalTaxInfo(modelist);
                }
                catch (Exception ex)
                {
                    //输出系统日志
