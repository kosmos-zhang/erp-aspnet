...
{
   public  class InsuPersonaIncomeTaxBus
    {
       public static DataTable GetInsuPersonalTaxInfo()
       {
           //获取登陆用户信息
           UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
           //执行查询
           return InsuPersonaIncomeTaxDBHelper.SearchInsuPersonalTaxInfo(userInfo.CompanyCD);
       }
       public static bool SaveInsuPersonInfo(IList <PercentIncomeTaxModel > modelist)
       {
           //定义返回变量
           bool isSucc = true;
           //信息存在时，进行操作
           if (modelist != null && modelist.Count  > 0)
           {
               ...
               {
                   //执行保存操作
                   isSucc = InsuPersonaIncomeTaxDBHelper.UpdateIsuPersonalTaxInfo(modelist);
               }
               catch (Exception ex)
               {
                   //输出系统日志
