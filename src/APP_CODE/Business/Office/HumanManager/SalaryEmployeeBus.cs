...
namespace XBase.Business.Office.HumanManager
{
    ...
    /// </summary>
    ///
    public class SalaryEmployeeBus
    {
        ...
        /// <returns></returns>
        public static bool SaveSalaryEmployeeInfo(ArrayList lstEdit)
        {
            //定义返回变量
            bool isSucc = false;
            //信息存在时，进行操作
            if (lstEdit != null && lstEdit.Count > 0)
            {
                ...
                {
                    //执行保存操作
                    isSucc = SalaryEmployeeDBHelper.SaveSalaryEmployeeInfo(lstEdit, userInfo.CompanyCD, userInfo.UserID);
                }
                catch (Exception ex)
                {
                    //输出系统日志
        ...
        /// </summary>
        /// <returns></returns>
        public static DataTable GetSalaryEmployeeInfo()
        {
            //获取登陆用户信息
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            //执行查询
            return SalaryEmployeeDBHelper.SearchSalaryEmployeeInfo(userInfo.CompanyCD);
        }
        #endregion
        ...
        private static LogInfoModel InitLogInfo(string no)
        {
            ...
            //设置操作日志类型 修改
            logModel.ObjectName = ConstUtil.CODING_RULE_TABLE_SALARY_EMPL;//操作对象
            //
