...
namespace XBase.Business.Office.HumanManager
{
    ...
    public class TimeItemBus
    {
        ...
        public static bool IsUsedItem(string itemNo)
        {
            ...
            //进行校验
            return TimeItemDBHelper.IsUsedItem(itemNo, companyCD);
        }
        #region 编辑计时工资项信息
        /// <summary>
        /// 编辑计时工资项信息
        ...
        /// <returns></returns>
        public static bool SaveTimeItemInfo(ArrayList lstEdit)
        {
            //定义返回变量
            bool isSucc = true;
            //信息存在时，进行操作
            if (lstEdit != null && lstEdit.Count > 0)
            {
                ...
                {
                    //执行保存操作
                    isSucc = TimeItemDBHelper.SaveTimeItemInfo(lstEdit, userInfo.CompanyCD, userInfo.UserID);
                }
                catch (Exception ex)
                {
                    //输出系统日志
        ...
        /// <returns></returns>
        public static DataTable GetTimeItemInfo(bool isUsed)
        {
            //获取登陆用户信息
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            //执行查询
            return TimeItemDBHelper.SearchTimeItemInfo(userInfo.CompanyCD, isUsed);
        }
        #endregion
        ...
        /// <returns></returns>
        public static DataTable GetTimeItemPrice(string itemNo)
        {
            //获取登陆用户信息
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            //执行查询
            return TimeItemDBHelper.GetTimeItemPrice(userInfo.CompanyCD, itemNo);
        }
        #endregion
        ...
        private static LogInfoModel InitLogInfo(string no)
        {
            ...
            //设置操作日志类型 修改
            logModel.ObjectName = ConstUtil.CODING_RULE_TABLE_TIME_ITEM;//操作对象
            //
