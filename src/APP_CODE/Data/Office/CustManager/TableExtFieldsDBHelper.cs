    ...
    public class TableExtFieldsDBHelper
    {
       ...
       /// <returns></returns>
        public static DataTable Getlist(string CompanyCD, string EFDesc, string EFType, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            sb.Append(" where CompanyCD=@CompanyCD and TabName='officedba.CustInfo' ");
        ...
        public static DataTable GetAllList(string CompanyCD)
        {
           // DataTable dt = new DataTable();
            ...
            sb.Append(" where CompanyCD=@CompanyCD and TabName='officedba.CustInfo'  ORDER BY EFType, ID asc ");
            ArrayList arr = new ArrayList();
            arr.Add(new SqlParameter("@CompanyCD", CompanyCD));
        ...
        public static DataTable GetAllListReport(string CompanyCD)
        {
            // DataTable dt = new DataTable();
            StringBuilder sb = new StringBuilder();
            sb.Append("select ID,EFIndex,EFDesc as ExtField1,EFType,EFValueList from officedba.TableExtFields");
            sb.Append(" where CompanyCD=@CompanyCD and TabName='officedba.CustInfo'  ORDER BY EFType, ID asc ");
            ArrayList arr = new ArrayList();
            arr.Add(new SqlParameter("@CompanyCD", CompanyCD));
        ...
        public static bool Delete(string strCompanyCD, string IDS, out string strMsg, out string strFieldText)
        {
            ...
            for (int i = 0; i < orderNoS.Length; i++)
            {
                ...
                {
                    strFieldText += "客户特性：" + SqlHelper.ExecuteScalar("select EFDesc from officedba.TableExtFields where ID=" + orderNoS[i], null).ToString() + "|";
                    strMsg += "已被使用不允许删除！|";
                    bTemp = true;
                }
        ...
        private static bool isDel(string id,string strCompanyCD)
        {
            ...
            strSql = "SELECT COUNT(1) FROM officedba.CustInfo WHERE (" + strKeyName + " IS NOT NULL) AND (CompanyCD = '" + strCompanyCD + "') AND (" + strKeyName + " <> '') ";
