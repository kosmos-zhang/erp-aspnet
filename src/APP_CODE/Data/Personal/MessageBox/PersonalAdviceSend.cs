...
namespace XBase.Data.Personal.MessageBox
{
	...
	public class PersonalAdviceSend
	{
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from [officedba].PersonalAdviceSend");
		...
		public void Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
            strSql.Append("delete [officedba].PersonalAdviceSend ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
			parameters[0].Value = ID;
		...
		/// 得到一个对象实体
		/// </summary>
		public XBase.Model.Personal.MessageBox.PersonalAdviceSend GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 ID,CompanyCD,AdviceType,FromUserID,DoUserID,Title,Content,CreateDate,DisplayName from [officedba].PersonalAdviceSend ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
			parameters[0].Value = ID;

			XBase.Model.Personal.MessageBox.PersonalAdviceSend model=new XBase.Model.Personal.MessageBox.PersonalAdviceSend();
		...
		public DataTable GetList(string strWhere)
		{
            ...
            strSql.Append(" FROM [officedba].PersonalAdviceSend ");
			if(strWhere.Trim()!="")
			{
        ...
        public int GetPageData(out DataTable dt, string where, string fields, string orderExp, int pageindex, int pagesize)
        {
            ...
            DataSet ds = SqlHelper.ExecuteDataset("", "[officedba].[PersonalAdviceSend_GetPageData]", prams);
            dt = ds.Tables[0];
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "PersonalAdviceSend";
