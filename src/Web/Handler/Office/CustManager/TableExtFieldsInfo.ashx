...
public class TableExtFieldsInfo : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (action == "insert")
        {
            isSucc = TableExtFieldsBus.Add(GetModel(context), out strMsg);
        }
        else if (action == "update")//保存后修改
        {
            isSucc = TableExtFieldsBus.Update(GetModel(context), out strMsg);
        }
        ...
        else if (action == "all")//获取列表
        {
            ...
            DataTable dt = TableExtFieldsBus.GetAllList(CompanyCD);
    ...
    private void DelOrder(HttpContext context)
    {
        ...
        string CompanyCD = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;//公司代码 
        JsonClass JC;
        if (TableExtFieldsBus.Delete(CompanyCD,orderNos, out strMsg, out strFieldText))
    ...
    private void GetLsit(HttpContext context)
    {
        ...
        DataTable dt = TableExtFieldsBus.Getlist(CompanyCD, EFDesc, EFType, pageIndex, pageCount, ord, ref totalCount);
