<%@ WebHandler Language="C#" Class="SalaryCommission" %>
/**********************************************
 * 作    者： 吴志强
 * 创建日期： 2009/05/06
 * 描    述： 提成工资设置
...
public class SalaryCommission : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (context.Request.QueryString["Action"] != null)
        {
            ...
            if ("IsDelete".Equals(action))
            {
                ...
                else
                {
                    //替换引号
                    ////校验是否被使用
                    bool isUsed = CommissionItemBus.IsUsedItem(itemNo);
                    //如果已经被使用
        ...
        else
        {
            ...
            //执行保存
            bool isSucc = CommissionItemBus.SaveCommissionItemInfo(lstEdit);
            //保存成功时
            if (isSucc)
            {
                //设置记录总数
        ...
        if (lstEdit != null && lstEdit.Count > 0)
        {
            ...
            for (int i = 0; i < lstEdit.Count; i++)
            {
                ...
                if ("0".Equals(model.EditFlag) || "1".Equals(model.EditFlag))
                {
                    ...
                    //单价
                    sbSalaryInfo.Append(",\"Rate\":\"" + model.Rate + "\"");
                    //启用状态
    ...
    private string EditRequstData(HttpRequest request, ArrayList lstReturn)
    {
        ...
        for (int i = 1; i <= salaryCount; i++)
        {
            ...
            if (model.ItemNo == "默认") model.ItemNo = null;
            //不是删除时
            if ("0".Equals(model.EditFlag) || "1".Equals(model.EditFlag))
            {
                //项目名称
                ...
                //提成率
                model.Rate = request.Params["Rate_" + i.ToString()].ToString();
                //启用状态
            ...
            {
                //校验是否被使用
                //bool isSucc = CommissionItemBus.IsUsedItem(model.ItemNo);
                ////如果已经被使用
                //if (isSucc)
                //{
                //    //返回
