<%@ WebHandler Language="C#" Class="PurchaseRejectDeptDetails" %>
...
public class PurchaseRejectDeptDetails : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    private DataTable GetDataTable(HttpContext context, int pageIndex, int pageCount, string ord,int pageindex,int pagesize, ref int TotalCount)
    {
        ...
        if (flag == "0") //根据部门统计
        {
            dt = PurchaseRejectBus.GetPurchaseRejectDeptDetails(companyCD, billStatus, FromType, Convert.ToInt32(shState), Convert.ToInt32(typeID), BeginDate, EndDate, ord, pageindex, pageCount, ref TotalCount);
        }
        else//根据供应商统计
        {
            dt = PurchaseRejectBus.GetPurchaseRejectProviderDetails(companyCD, billStatus, FromType, Convert.ToInt32(shState), Convert.ToInt32(typeID), BeginDate, EndDate, ord, pageindex, pageCount, ref TotalCount);
        }
        return dt;
    }    
