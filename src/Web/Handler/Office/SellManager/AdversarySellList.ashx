<%@ WebHandler Language="C#" Class="AdversarySellList" %>
...
public class AdversarySellList : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
     ...
     public void ProcessRequest(HttpContext context)
    {
        if (context.Request.RequestType == "POST")
        {
            ...
            else if (action == "orderInfo")//获取销售机会详细信息
            {
                ...
                DataTable dt = AdversarySellBus.GetOrderInfo(orderID);
                strJson = "{";
                if (dt.Rows.Count > 0)
                {
    ...
    private void DelOrder(HttpContext context)
    {
        ...
        JsonClass JC;
        if (AdversarySellBus.DelOrder(orderNos))
    ...
    private DataTable GetDataTable(HttpContext context, int pageIndex, int pageCount, string ord, ref int TotalCount)
    {
        ...
        string strChanceID = context.Request.Params["ChanceID"].ToString().Trim();
