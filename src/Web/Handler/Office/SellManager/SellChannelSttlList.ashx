<%@ WebHandler Language="C#" Class="SellChannelSttlList" %>
...
public class SellChannelSttlList : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    public void ProcessRequest(HttpContext context)
    {
        if (context.Request.RequestType == "POST")
        {
            ...
            else if (action == "orderInfo")//获取单据详细信息
            {
                ...
                DataTable dt = SellChannelSttlBus.GetOrderInfo(orderID);
                strJson = "{";
                if (dt.Rows.Count > 0)
                {
            ...
            else if (action == "detail")//获取单据明细信息
            {
                ...
                string strJson = string.Empty;
                DataTable dt = SellChannelSttlBus.GetOrderDetail(orderNo);
                strJson = "{";
                if (dt.Rows.Count > 0)
                {
                    strJson += "\"data\":" + JsonClass.DataTable2Json(dt);
    ...
    private void DelOrder(HttpContext context)
    {
        ...
        JsonClass JC;
        if (SellChannelSttlBus.DelOrder(orderNos, out strMsg, out strFieldText))
