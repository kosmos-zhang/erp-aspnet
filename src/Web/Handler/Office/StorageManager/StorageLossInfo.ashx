<%@ WebHandler Language="C#" Class="StorageLossInfo" %>
...
public class StorageLossInfo : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        if (context.Request.RequestType == "POST")
        {
            ...
            else
            {
                ...
                model.ID = ID;
                DataTable DT = StorageLossBus.GetStorageLossDetailInfo(model);
    ...
    private void CloseBill(HttpContext context)
    {
        ...
        if (context.Request.QueryString["Act"].ToString().Trim() == "Close")
            if (ID > 0)
            {
                ...
                if (StorageLossBus.CloseBill(model))
                {
    ...
    private void CancelCloseBill(HttpContext context)
    {
        ...
        if (ID > 0)
        {
            ...
            model.Closer = loginUser_id.ToString();
            model.ModifiedUserID = LoginUserID;
            if (StorageLossBus.CancelCloseBill(model))
            {
