<%@ WebHandler Language="C#" Class="CompareSellPrice" %>
...
public class CompareSellPrice : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        if (context.Request.RequestType == "POST")
        {
            ...
            string Product = context.Request.Form["Product"].ToString();
            string BeginDate = context.Request.Form["BeginDate"].ToString();
            string EndDate = context.Request.Form["EndDate"].ToString();
            string QueryStr = " 1=1 ";
            if (Product != "0" && Product != "")
            {
                QueryStr += " and a.ProductID=" + int.Parse(Product) + "";
            }
            if (BeginDate!="")
            {
                QueryStr += " and b.OrderDate>='"+BeginDate+"'";
            }
            if (EndDate!="")
            {
                QueryStr += " and b.OrderDate<='"+EndDate+"'";
            }
            string myOrderby = orderBy+" "+order;
            int TotalCount=0;
            DataTable dt=SellProductReportBus.GetSellPriceDB(QueryStr,pageIndex,pageCount,myOrderby,ref TotalCount);
