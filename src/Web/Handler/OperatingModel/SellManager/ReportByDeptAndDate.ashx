<%@ WebHandler Language="C#" Class="ReportByDeptAndDate" %>
...
public class ReportByDeptAndDate : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    private void GetLsit(HttpContext context)
    {
        ...
        if (strCurrencyType == "")
        {
            dt = SellOrderBus.ReportByDeptAndDate(SellDeptId, StartDate, EndDate, "2", pageIndex, pageCount, ord, ref totalCount);
        }
        else
        {
            dt = SellOrderBus.ReportByDeptAndDate(SellDeptId, StartDate, EndDate,Convert.ToInt32(strCurrencyType), "2", pageIndex, pageCount, ord, ref totalCount);
        }
