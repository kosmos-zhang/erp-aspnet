<%@ WebHandler Language="C#" Class="SubSellProductCompare" %>
...
public class SubSellProductCompare : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    private void GetSellProductLsit(HttpContext context)
    {
        ...
        string SellendDate_ = context.Request.Params["SellEmdDate"].ToString().Trim();
        string ProdID = context.Request.Params["ProductID"].ToString().Trim();
        DateTime SellDate = Convert.ToDateTime(SellDate_);
        DateTime sellEndDate = Convert.ToDateTime(SellendDate_);
        dt = SubStorageBus.GetSubProductSellInfo(SellDate, sellEndDate, ProdID, pageIndex, pageCount, ord, ref totalCount);
