...
public partial class Pages_Office_HumanManager_test : System.Web.UI.Page
{
    #region 私有成员
    //显示类别 1部门 2人员
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            if (rows.Length > 0)
            {
                for (int i = 0; i < rows.Length; i++)
                {
                    ...
                    node.Text = "  <img  src=\"../../../Images/folder-closed.gif\" border=\"0\" /><span style=\"font-size:15px\"> " + rows[i]["DeptName"].ToString() + " </span>"; 
                    ...
                    BindChildNode(node.Value, treenode, dt, Userdt);
                }
            }
        }
    }
