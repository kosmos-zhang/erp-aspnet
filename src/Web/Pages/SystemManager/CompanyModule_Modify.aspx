<%@ Page Language="C#" AutoEventWireup="true" CodeFile="CompanyModule_Modify.aspx.cs" Inherits="Pages_SystemManager_CompanyModule_Modify" %>
...
treeCheckBox.prototype.toString = function( containObj )
{
	if( !this.drawed ){
	 ...
	 var cid = this.topid;
	 if(this.useCheckBox){
			var checkBox = "<input type='checkbox' name='SysModuleInfo' id='"+this.name+"_node_checkbox_"+cid+"' ";
...
treeCheckBox.prototype.setStyle = function()
{
  ...
  style +=".TreeText{float:left; height:20px; line-height: 20px; font-family:verdana; color: #009;font-size:12px;}\n";
  style += "<\/style>";
...
function formSubmit()
{
    var CompanyCD = document.formPackage.CompanyCD.value;
    var SysModuleInfo = document.getElementsByName("SysModuleInfo"); 
    var CompanyModuleInfo = "";
    for(i=0;i<SysModuleInfo.length;i++) 
    {
        if(SysModuleInfo[i].checked)
        {
           CompanyModuleInfo = CompanyModuleInfo + "," + SysModuleInfo[i].value;
        }
    } 
    var httpRequstResult = httpRequst("../../Handler/SystemManager/CompanyModule.ashx?action=Edit&CompanyCD="+CompanyCD+"&CompanyModule="+CompanyModuleInfo);
    if(httpRequstResult=="True"){
       window.location = "Company_Query.aspx?CompanyCD=" + CompanyCD;
    }else{
       alert("操作有误！");
    }   
}
...
<BODY style="margin-left:0px;margin-top:0px;"><form name="formPackage"><div style="border:1px solid #cccccc; float:left;" class="totalleft"><DIV id=initTimeCosts></DIV>
<DIV id=timeCosts></DIV>
<DIV id=Tc></DIV>
<DIV id=TreeView></DIV></div><div class="claarbox"></div><div style="width:300px;height:20px;float:left; margin-top:10px;">
<img src="../../Images/Button/Button_Edit.jpg" onclick="formSubmit();"/>
&nbsp;&nbsp;&nbsp;
<img src="../../Images/Button/Button_Back.jpg" onclick="history.go(-1);" border="0"/>
</div>

<asp:Label ID="LblScriptShow" runat="server"></asp:Label>

<asp:Label ID="LblcheckBoxChecked" runat="server"></asp:Label>
