<%@ Control Language="C#" AutoEventWireup="true" CodeFile="SellModuleSelectFeeTypeUC.ascx.cs"
    Inherits="UserControl_SellModuleSelectFeeTypeUC" %>
<div id="sellModuleFeeType">
    <!--提示信息弹出详情start-->
    <a name="pageFeeTypeMark"></a>
    <div id="divSellModuleFeeTypeSelect" style="border: solid 10px #93BCDD; background: #fff;
        padding: 10px; width: 700px; z-index: 21; position: absolute; display: none;
        top: 80%; left: 60%; margin: 5px 0 0 -400px;">
        <table width="100%">
            <tr>
                <td align="left">
                 <img src="../../../Images/Button/Bottom_btn_close.jpg" alt="关闭" onclick="closeSellModuFeeTypediv()" />
            
                </td>
            </tr>
        </table>
        <table width="99%" border="0" align="center" cellpadding="0" cellspacing="1" id="sellFeeTypeList"
            bgcolor="#999999">
            <tbody>
                <tr>
...
popSellFeeTypeObj.ShowList=function(objInput1)
{
    ...
    function SellFeeTurnToPage(pageIndex,objInput1)
    {
           ...
           success: function(msg){
                    //数据获取完毕，填充页面据显示
                    //数据列表
                    $("#sellFeeTypeList tbody").find("tr.newrow").remove();
                    $.each(msg.data,function(i,item){
                        ...
                        $("<tr class='newrow'></tr>").append("<td height='22' align='center'>"+" <input type=\"radio\" name=\"radioFee_\" id=\"radioFee_"+item.ID+"\" value=\""+item.ID+"\" onclick=\"fnSelectSellFee("+item.ID+",'"+item.CodeName+"','"+objInput1+"');\" />"+"</td>"+
                         ...
                         "<td height='22' align='center'>"+item.Description+"</td>").appendTo($("#sellFeeTypeList tbody"));
                   });
                    //页码
           ...
           error: function() {}, 
           complete:function(){$("#sellFeeList_Pager").show();pageSellFeeDataList1("sellFeeTypeList","#E7E7E7","#FFFFFF","#cfc","cfc");}//接收数据完毕
           });
    }
    //table行颜色
...
function fnSelectSellFee(ID,CodeName,objInput1)
{
    ...
    $("#"+popSellFeeTypeObj.InputObj1).val(CodeName);
    $("#"+popSellFeeTypeObj.InputObj1).attr("title",ID);
