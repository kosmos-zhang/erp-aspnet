...
$(document).ready(function() {
   TurnToPage(1);
    ...
    {
         action = "ReportCustTime";//客户分类对比
         currentPageIndex = pageIndex;
         requestobj = GetRequest(); 
         var DateTimeID = requestobj['XValue'];
         var DateSele = requestobj['DateSele'];
         var CustTypeManage = requestobj['CustTypeManage'];
         var CustTypeSell = requestobj['CustTypeSell'];
         var CustTypeTime = requestobj['CustTypeTime'];         
         ...
         var EndDate = requestobj['EndDate'];
           ...
           cache:false,
           data: "pageIndex="+currentPageIndex+"&pageCount="+pageCount+"&action="+action+"&orderby="+orderBy+"&DateTimeID="+reescape(DateTimeID)+
                            "&DateSele="+reescape(DateSele)+"&CustTypeManage="+reescape(CustTypeManage)+"&CustTypeSell="+reescape(CustTypeSell)+
                            "&CustTypeTime="+reescape(CustTypeTime)+"&CustType="+reescape(CustType)+"&CustClass="+reescape(CustClass)+"&Area="+reescape(Area)+"&RelaGrade="+reescape(RelaGrade)+
                            "&CreditGrade="+reescape(CreditGrade)+"&BeginDate="+reescape(BeginDate)+"&EndDate="+reescape(EndDate),//数据
