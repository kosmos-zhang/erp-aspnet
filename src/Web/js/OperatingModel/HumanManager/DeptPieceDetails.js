...
function TurnToPage(pageIndex)
{
       ...
       var iyear=document .getElementById ("ddlTaskYear").value;//发文类型      
  
              document.getElementById("hdPara").value="1";
   //document.getElementById("hdPara").value = "orderby="+orderBy+"&DeptID="+escape(DeptID)+"&year="+escape(iyear )   ;
 
      action ="DeptPieceDetails_Select";
       $.ajax({
       type: "POST",//用POST方式传输
       dataType:"json",//数据格式:JSON
       url:  '../../../Handler/OperatingModel/HumanManager/HumanReport.ashx',//目标地址
       cache:false,
       data: "pageIndex="+pageIndex+"&pageCount="+pageCount+"&Action="+action+"&orderby="+orderBy+"&DeptID="+escape(DeptID)+"&year="+escape(iyear )   ,//数据
       ...
       success: function(msg){
                ...
                $("#pageDataList1 tbody").find("tr.newrow").remove();
                  ...
                  if(item.EmployeeID != null && item.EmployeeID != "")
                 {
                    ...
                    "<td height='22' align='center'>" + item.ReportMonth + "</td>"+   
                    "<td height='22' align='center'>"+item.WorkMoney+"</td>").appendTo($("#pageDataList1 tbody"));}
               });
