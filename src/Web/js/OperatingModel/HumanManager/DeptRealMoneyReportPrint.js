...
function TurnToPage(pageIndex)
{
       ...
       var  Year= document .getElementById ("hidYear").value;//发文类型      
      var sd= Year.replace('年','');
        var sdfdf=    sd.replace('月','');
                
       var iyear=sdfdf.substring(0,4);
       var month=sdfdf.replace(iyear,'')
...
//       document.getElementById("hdPara").value = "orderby="+orderBy+"&quterID="+escape(quterID)+"&QuaterAdmin="+escape (QuaterAdmin)  ;
      ...
      action ="DeptRealMoneyReportPrint_Select";
       $.ajax({
       type: "POST",//用POST方式传输
       dataType:"json",//数据格式:JSON
       url:  '../../../Handler/OperatingModel/HumanManager/HumanReport.ashx',//目标地址
       cache:false,
       data: "pageIndex="+pageIndex+"&pageCount="+pageCount+"&Action="+action+"&orderby="+orderBy+"&DeptID="+escape(DeptID)+"&Month="+escape (month)+"&year="+escape(iyear )   ,//数据
