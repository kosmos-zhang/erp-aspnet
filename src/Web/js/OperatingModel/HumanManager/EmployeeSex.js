...
function TurnToPage(pageIndex)
{
       ...
       var DeptID = document.getElementById("txtDeptID").value;//发文类型      
        
       document.getElementById("hdPara").value = "orderby="+orderBy+"&DeptID="+escape(DeptID);
      
       $.ajax({
       type: "POST",//用POST方式传输
       dataType:"json",//数据格式:JSON
       url:  '../../../Handler/OperatingModel/HumanManager/EmployeeSex.ashx',//目标地址
       cache:false,
       data: "pageIndex="+pageIndex+"&pageCount="+pageCount+"&action="+action+"&orderby="+orderBy+"&DeptID="+escape(DeptID),//数据
       ...
       success: function(msg){
                ...
                $.each(msg.data,function(i,item){
                  if(item.DeptID != null && item.DeptID != "")
                 {
                    ...
                    "<td height='22' align='center'>"+ item.ManCount +"</td>"+
                    "<td height='22' align='center'>" + item.WoManCount + "</td>"+                   
