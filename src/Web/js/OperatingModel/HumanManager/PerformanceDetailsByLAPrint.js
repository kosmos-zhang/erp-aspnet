  $(document).ready(function(){
    ...
    var PerType = requestobj['PerType'];
    var TaskNum = requestobj['TaskNum'];
    var LevelType = requestobj['LevelType'];
    var TaskYear = requestobj['TaskYear'];
    if(typeof(LevelType)!="undefined")
    { 
    document .getElementById ("hdPara").value="deptid="+escape (deptid)+"&TaskFlag="+escape ( TaskFlag)+"&TaskNo="+escape ( TaskNo)+"&PerType="+escape ( PerType) +"&TaskNum="+escape (TaskNum)+"&LevelType="+      escape ( LevelType)+"&TaskYear="+escape (TaskYear); 
    
      document .getElementById ("hidDeptID").value=deptid;
      document .getElementById ("hidTaskFlag").value=TaskFlag;
      document .getElementById ("hidTaskNo").value=TaskNo;
      document .getElementById ("hidPerType").value=PerType;
      document .getElementById ("hidTaskNum").value=TaskNum;
      document .getElementById ("hidLevelType").value=LevelType;
      document .getElementById ("hidTaskYear").value=TaskYear;
...
//       document.getElementById("hdPara").value = "orderby="+orderBy+"&quterID="+escape(quterID)+"&QuaterAdmin="+escape (QuaterAdmin)  ;
      ...
      action ="PerformanceDetailsByLAPrint_Select";
       $.ajax({
       type: "POST",//用POST方式传输
                ...
                $("#pageDataList1 tbody").find("tr.newrow").remove();
                  ...
                  if(item.EmployeeID != null && item.EmployeeID != "")
                 {
                    ...
                    "<td height='22' align='center'>" + item.Title + "</td>"+  
                            "<td height='22' align='center'>" + item.FlagName + "</td>"+  
                    "<td height='22' align='center'>"+ item.EmployeeName +"</td>"+ 
                    "<td height='22' align='center'>"+item.LevelType+"</td>").appendTo($("#pageDataList1 tbody"));}
               });
