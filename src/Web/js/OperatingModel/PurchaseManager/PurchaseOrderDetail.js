...
function TurnToPage(pageIndex) {
    ...
    var OrderStatus=$("#HOrderStatus").val();
    var FlowStatus=$("#HFlowStatus").val();
    var FromType=$("#HFromType").val();
    var Type=$("#HType").val();
    ...
    var strUrl ='OrderStatus='+OrderStatus+'&FlowStatus='+FlowStatus+'&FromType='+escape(FromType)+'&Type='+escape(Type)+ '&DateType='+escape(DateType)+ '&BeginDate='+escape(BeginDate)+ '&EndDate='+escape(EndDate)+ '&DeptId='+escape(DeptId)+ '&ProviderId='+escape(ProviderId)+ '&DateValue='+escape(DateValue)+'&pageIndex=' + pageIndex + '&pageSize=' + pageCount + '&orderby=' + escape(orderBy) +'&action='+Action;
        
    $.ajax({

        type: "POST", //用POST方式传输
        dataType: "json", //数据格式:JSON
        url: '../../../Handler/OperatingModel/PurchaseManager/PurchaseOrder.ashx', //目标地址
        cache: false,
        data: strUrl,
        ...
        success: function(msg) {
                    ...
                    $.each(msg.data,function(i,item)
                    {
                        ...
                        "<td height='22' align='center'>"+ (parseFloat(item.TotalTax)).toFixed($("#hidPoint").val()) +"</td>"+ 
                        ...
                        "<td height='22' align='center' id='FlowStatusName"+(i + 1)+"'>"+ item.isOpenBill +"</td>"
