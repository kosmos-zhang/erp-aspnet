...
//    orderBy=orderBy.split('_')[0];
    ...
    var ProductID = document.getElementById("txtHidProductID").value;
    var StartOrderDate = document.getElementById("txtStartOrderDate").value;
    var EndOrderDate = document.getElementById("txtEndOrderDate").value;
            
//    var strRul = $("#hiddSearch").val();
    window.open("PurchasePriceAnalysePrint.aspx?pageIndex="+currentPageIndex+"&pageCount="+pageCount+"&orderby="+orderBy+"&ProductID="+escape(ProductID)+
                 "&StartOrderDate="+escape(StartOrderDate)+"&EndOrderDate="+escape(EndOrderDate)+"");
...
//            var EndOrderDate = document.getElementById("txtEndOrderDate").value;
           ...
           url:  '../../../Handler/OperatingModel/PurchaseManager/PurchasePriceAnalyse.ashx',//目标地址
           cache:false,
...
//                        "<td height='22' align='center'><a href='" + GetLinkParam(ProductID) +"&ProductID=" + item.ProductID + " ')>"+item.ProductNo+"</a></td>"+
                        ...
                        "<td height='22' align='center'>"+ item.CompanyCD +"</a></td>"+
                        "<td height='22' align='center'>"+ item.OrderNo +"</a></td>"+
                        "<td height='22' align='center'>"+ FormatAfterDotNumber(item.Rate,4) +"</a></td>"+
                        "<td height='22' align='center'>"+ FormatAfterDotNumber(item.TotalPrice,2) +"</a></td>"+
                        "<td height='22' align='center'>"+ FormatAfterDotNumber(item.TotalTax,2) +"</a></td>"+
                        "<td height='22' align='center'>"+ item.Title +"</a></td>"+
                        "<td height='22' align='center'>"+ item.TheyDelegate +"</a></td>"+
                        "<td height='22' align='center'>"+ FormatAfterDotNumber(item.TotalFee,2) +"</a></td>"+
                        "<td height='22' align='center'>"+ item.ProviderBillID +"</a></td>"+
                        "<td height='22' align='center'>"+ item.remark +"</a></td>").appendTo($("#pageDataList1 tbody"));
                        }
...
function fnCheck()
{
    ...
    if(document.getElementById("txtStartOrderDate").value == "")
    {
        isFlag = false;
        fieldText += "起始日期|";
   		msgText +=  "起始日期不能为空|";  
    }
    if(document.getElementById("txtEndOrderDate").value == "")
    {
        isFlag = false;
        fieldText += "结束日期|";
   		msgText +=  "结束日期不能为空|";  
...
//采购历史价格列表
function SearchPurchaseProviderCount()
    {
    if(!fnCheck())
    return;
//检索条件
...
//    currentPageIndex = pageIndex;
    var Isliebiao = 1;
    var URLParams = "&Isliebiao="+escape(Isliebiao)+"&ProductID="+escape(ProductID)+
                     "&StartOrderDate="+escape(StartOrderDate)+"&EndOrderDate="+escape(EndOrderDate)+""; 
    //设置检索条件
...
{
    document.getElementById("txtHidProductID").value = id;
