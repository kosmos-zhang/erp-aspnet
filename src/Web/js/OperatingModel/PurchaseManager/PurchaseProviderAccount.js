...
//    var strRul = $("#hiddSearch").val();
    window.open("PurchaseProviderAccountPrint.aspx?pageIndex="+currentPageIndex+"&pageCount="+pageCount+"&orderby="+orderBy+"&ProviderID="+escape(ProviderID)+
...
//           var EndConfirmDate = document.getElementById("txtEndConfirmDate").value;
           ...
           url:  '../../../Handler/OperatingModel/PurchaseManager/PurchaseProviderAccount.ashx',//目标地址
           cache:false,
...
//                        "<td height='22' align='center'><a href='" + GetLinkParam(ProductID) +"&ProductID=" + item.ProductID + " ')>"+item.ProductNo+"</a></td>"+
                        ...
                        "<td height='22' align='center'>"+ item.SignAddr +"</a></td>"+
                        "<td height='22' align='center'>"+ item.SignDate +"</a></td>"+
                        "<td height='22' align='center'>"+ item.Note +"</a></td>"+
                        "<td height='22' align='center'>" + (parseFloat(item.TotalPrice)).toFixed($("#hidPoint").val()) + "</a></td>").appendTo($("#pageDataList1 tbody"));
                        }
...
//采购历史价格列表
function SearchPurchaseProviderAccount()
    {
    if(!fnCheck())
    return;
//检索条件
