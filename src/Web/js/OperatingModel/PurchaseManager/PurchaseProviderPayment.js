var pageCount = 10;//每页计数
    ...
    var action = "";//操作
    var orderByFFF = "ProviderNo ASC";//排序字段
    var Isliebiao ;
...
//        ordering="desc"
//    }
//    orderByFFF=orderByFFF.split('_')[0];
    
    var ProviderID = document.getElementById("txtHidProviderID").value;
   var ProviderName = document.getElementById("txtProviderName").value.Trim();
...
//    var strRul = $("#hiddSearch").val();
    window.open("PurchaseProviderPaymentPrint.aspx?pageIndex="+currentPageIndex+"&pageCount="+pageCount+"&orderby="+orderByFFF+"&ProviderID="+escape(ProviderID)+
                 "&ProviderName="+escape(ProviderName)+"&StartConfirmDate="+escape(StartConfirmDate)+"&EndConfirmDate="+escape(EndConfirmDate)+"");
...
//           var EndConfirmDate = document.getElementById("txtEndConfirmDate").value;
           ...
           url:  '../../../Handler/OperatingModel/PurchaseManager/PurchaseProviderPayment.ashx',//目标地址
           cache:false,
           data: "pageIndex="+pageIndex+"&pageCount="+currentpageCount+"&orderby="+orderByFFF+
...
//                        "<td height='22' align='center'><a href='" + GetLinkParam(ProductID) +"&ProductID=" + item.ProductID + " ')>"+item.ProductNo+"</a></td>"+
                        ...
                        "<td height='22' align='center'>"+ item.BillNo +"</a></td>"+
                        ...
                        "<td height='22' align='center'>"+ (parseFloat(item.YAccounts)).toFixed($("#hidPoint").val())+"</a></td>"+
                        "<td height='22' align='center'>"+ (parseFloat(item.NAccounts)).toFixed($("#hidPoint").val()) +"</a></td>").appendTo($("#pageDataList1 tbody"));
                        }
...
function fnCheck()
{
    ...
    if(strlen(ProviderName)>100)
    {
        isFlag = false;
        fieldText += "供应商名称|";
   		msgText +=  "供应商名称仅限于100个字符以内|";      
    }
...
//采购历史价格列表
function SearchPurchaseProviderPayment()
    {
    if(!fnCheck())
    return;
//检索条件
...
//    currentPageIndex = pageIndex;
    var Isliebiao = 1;
    var URLParams = "&Isliebiao="+escape(Isliebiao)+"&ProviderID="+escape(ProviderID)+"&ProviderName="+escape(ProviderName)+
...
function Ifshow(count)
    ...
    if(issearch=="")
        ...
        orderByFFF = orderColum+" "+ordering;
        TurnToPage(1);
    }
