...
var orderBy = "ProductID ASC"; //排序字段
...
function fnGetSearch() {
    ...
    var strUrl = "";
    var ProductID = $("#txtProdID").val();          //单据状态
    var StartDate = $.trim($("#txtStartDate").val()); //开始日期
    var EndDate = $.trim($("#txtEndDate").val()); //截止日期

    strUrl = 'ProductID=' + escape(ProductID) + '&StartDate=' + escape(StartDate) + '&EndDate=' + escape(EndDate);
...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/PurchaseManager/PurchaseStatistByProduct.ashx', //目标地址
        ...
        success: function(msg) {
            ...
            if (msg != null) {
                if (msg.data.length != 0) {
                    ...
                    $("<tr class='newrow'></tr>").append("<td height='22' align='center'>" + item.ProductNo + "</td>"
...
//    }
    ...
    window.open("PurchaseStatistByProductPrint.aspx?orderby="+orderBy+"&" + strRul);
}
