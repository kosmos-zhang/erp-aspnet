...
{//检索
    ...
    var Product=document.getElementById('theProductID').value;
    var BeginDate=document.getElementById('theBeginDate').value;
    var EndDate=document.getElementById('theEndDate').value;
    ...
    var MyData = "pageIndex="+pageIndex+"&pageCount="+pageCount+"&action="+action+"&orderBy="+orderBy+
                     "&Product="+escape(Product)+"&BeginDate="+escape(BeginDate)+"&EndDate="+escape(EndDate); 

    $.ajax({
           type: "POST",//用POST方式传输
           dataType:"json",//数据格式:JSON
           url:  '../../../Handler/OperatingModel/SellManager/CompareSellPrice.ashx',//目标地址
           cache:false,
           data: MyData,//数据
           ...
           success: function(msg){
                    ...
                    var j =1;
                      $.each(msg.data,function(i,item){
                        if(item.TotalFee != null && item.TotalFee != "")
                        ...
                        "<td height='22' align='right'>"+parseFloat(item.TotalFee).toFixed(2)+"</td>").appendTo($("#tblDetailInfo tbody"));                      
...
//                             +"<td height='22' align='center'>"+msg.TotalLastYear+"</td><td height='22' align='center'>"+msg.TotalPrice+"</td>").appendTo($("#tblDetailInfo tbody")); 
                             //页码
            if(parseFloat(msg.totalCount)>0)
              {
...
function PrintSpreadsheetBalanceTotal()
{
    ...
    var BeginDate=document.getElementById('theBeginDate').value;
    var EndDate=document.getElementById('theEndDate').value;
    var searchStr="orderBy="+orderBy+"&Product="+Product+"&BeginDate="+BeginDate+"&EndDate="+EndDate;
    window.open("PrintCompareSellPrice.aspx?"+searchStr);
}
function GetFloat(a)
{
   if(a!=null && a!='')
