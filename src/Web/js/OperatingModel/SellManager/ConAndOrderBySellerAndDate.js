...
function fnGetSearch() {
    ...
    var strUrl = "";
    var startDate = $.trim($("#ddlStartDate").val()) + '-' + $.trim($("#ddlStartMonth").val()); //开始日期
    var endDate = $.trim($("#ddlEndDate").val()) + '-' + $.trim($("#ddlEndMonth").val()); //截止日期
    var Seller = $("#Seller").val();          //业务员ID（对应员工表ID）
    var isCon = document.getElementById("chkCon").checked; //合同统计
    var isOrd = document.getElementById("chkOrd").checked; //订单统计
    ...
    strUrl = 'isCon=' + escape(isCon) + '&startDate=' + escape(startDate) + '&endDate=' + escape(endDate)
             + '&isOrd=' + escape(isOrd) + '&Seller=' + escape(Seller) + '&CurrencyType=' + escape(CurrencyType) + '&CurrencyTypeName=' + escape(CurrencyTypeName);
...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/SellManager/ConAndOrderBySellerAndDate.ashx', //目标地址
                ...
                if (msg.data.length != 0) {
                    ...
                    if (item.EmployeeName != null && item.EmployeeName != "") {
                     ...
                     + "<td height='22' align='center'>" + item.OrderDate + "</td>"
                        + "<td height='22' align='right'>" + FormatAfterDotNumber(item.CountTotal, 2) + "</td>"
                        + "<td height='22' align='right'>" + FormatAfterDotNumber(item.TotalPrice, 2) + "</td>"
...
function PrintSubjectsTotal() {
    ...
    window.open("PrintConAndOrderBySellerAndDate.aspx?" + strRul);
}
...
function CheckInput() {
    ...
    var FindDate = $.trim($("#ddlStartDate").val()) + '-' + $.trim($("#ddlStartMonth").val()) + '-1'; //开始日期

    var FindDate1 = $.trim($("#ddlEndDate").val()) + '-' + $.trim($("#ddlEndMonth").val()) + '-1'; //发现日期
    var isCon = document.getElementById("chkCon").checked; //合同统计
    var isOrd = document.getElementById("chkOrd").checked; //订单统计
    if (!(document.getElementById("chkCon").checked) && !(document.getElementById("chkOrd").checked)) {
        isFlag = false;
        fieldText = fieldText + "统计方式|";
        msgText = msgText + "请至少选择一种统计方式|";
    }
    if (FindDate == '') {
        isFlag = false;
