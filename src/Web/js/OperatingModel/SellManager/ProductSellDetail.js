...
{//检索
    ...
    var MyData = "pageIndex="+pageIndex+"&pageCount="+pageCount+"&action="+action+"&orderBy="+orderBy+
                     "&ProductID="+escape(ProductID)+"&BeginDate="+escape(BeginDate)+"&EndDate="+escape(EndDate)+""; 
    var search = "";
    ...
    $.ajax({
           ...
           url:  '../../../Handler/OperatingModel/SellManager/ProductSellDetail.ashx',//目标地址
           cache:false,
           data: MyData,//数据
           ...
           success: function(msg){
                    ...
                    var j =1;
                      $.each(msg.data,function(i,item){
                        ...
                        "<td height='22' align='right'>"+(parseFloat(item.BackCount)).toFixed($("#hidPoint").val())+"</td>"+
                        "<td height='22' align='right'>"+(parseFloat(item.BackPrice)).toFixed($("#hidPoint").val())+"</td>").appendTo($("#tblDetailInfo tbody"));                      
                        
                   });
//                     $("<tr class='newrow' ></tr>").append("<td height='22' align='center'>合计</td>"
//                           +"<td height='22' align='center'>"+msg.TotalProductCount+"</td>"
//                           +"<td height='22' align='center'>"+msg.TotalPrice+"</td>"
//                           +"<td height='22' align='center'>"+msg.TotalBackCount+"</td>"
//                             +"<td height='22' align='center'>"+msg.TotalBackPrice+"</td>").appendTo($("#tblDetailInfo tbody")); 
                             //页码
            ShowPageBar(
...
function PrintSpreadsheetBalanceTotal()
{
    ...
    window.open("PrintProductSellDetail.aspx?type=1"+searchStr);
}
function GetFloat(a)
{
   if(a!=null && a!='')
