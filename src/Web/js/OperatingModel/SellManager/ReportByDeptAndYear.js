...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/SellManager/ReportByDeptAndYear.ashx', //目标地址
            ...
            if (msg != null) {
                if (msg.data.length != 0) {
                    ...
                    $.each(msg.data, function(i, item) {
                        ...
                        + "<td height='22' align='right'>" + FormatAfterDotNumber(item.UpC, 2) + "</td>"
                        + "<td height='22' align='right'>" + FormatAfterDotNumber(item.UpR, 2) + "</td>"
                        + "<td height='22' align='right'>" + FormatAfterDotNumber(item.NowC, 2) + "</td>"
                        + "<td height='22' align='right'>" + FormatAfterDotNumber(item.NowR, 2) + "</td>"
...
function PrintSubjectsTotal() {
    ...
    window.open("PrintReportByDeptAndYear.aspx?" + strRul);
}
...
function CheckInput() {
    ...
    if (OrderDate == '') {
        isFlag = false;
        fieldText = fieldText + "销售年度|";
        msgText = msgText + "请选择销售年度|";
    }
