...
{//检索
    ...
    var MyData = "pageIndex="+pageIndex+"&pageCount="+pageCount+"&action="+action+"&orderBy="+orderBy+
                     "&Area="+escape(Area)+"&theDate="+escape(theDate); 

    $.ajax({
           type: "POST",//用POST方式传输
           dataType:"json",//数据格式:JSON
           url:  '../../../Handler/OperatingModel/SellManager/SellAchievementGrowing.ashx',//目标地址
           cache:false,
           data: MyData,//数据
           ...
           success: function(msg){
                    ...
                    var j =1;
                      $.each(msg.data,function(i,item){
                        ...
                        "<td height='22' align='right'>"+parseFloat(item.TotalPrice).toFixed(2)+"</td>").appendTo($("#tblDetailInfo tbody"));                      
                        
                   });
//                     $("<tr class='newrow' ></tr>").append("<td height='22' align='center'>合计</td>"
//                           +"<td height='22' align='right'>"+msg.TotalNowMonth+"</td>"
...
function PrintSpreadsheetBalanceTotal()
{
    var Area=document.getElementById('ddlArea').value;
    var theDate=document.getElementById('theDate').value;
    var searchStr="orderBy="+orderBy+"&Area="+Area+"&theDate="+theDate+"&pageIndex="+currentPageIndex+"&pageCount="+pageCount;
    window.open("PrintSellAchievementGrowing.aspx?"+searchStr);
}
function GetFloat(a)
{
   if(a!=null && a!='')
