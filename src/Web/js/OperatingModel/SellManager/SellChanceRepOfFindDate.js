...
function fnGetSearch() {
    ...
    var strUrl = "";
    var FindDate = $.trim($("#ddlStartDate").val()) + '-' + $.trim($("#ddlStartMonth").val()); //开始日期

    var FindDate1 = $.trim($("#ddlEndDate").val()) + '-' + $.trim($("#ddlEndMonth").val()); //发现日期
    var total = $("#total").val(); //统计方式
    if (total == "SellDeptId") {
        $("#lblTitle").html("部门");
    }
    else {
    ...
    strUrl = 'total=' + escape(total) + '&FindDate=' + escape(FindDate) + '&FindDate1=' + escape(FindDate1);
...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/SellManager/SellChanceRepOfFindDate.ashx', //目标地址
            ...
            if (msg != null) {
                if (msg.data.length != 0) {
                    ...
                    if (item.title != null && item.title != "") {
                            $("<tr class='newrow'></tr>").append("<td height='22' align='center'>" + item.title + "</td>"
...
function PrintSubjectsTotal() {
    ...
    window.open("PrintSellChanceRepOfFindDate.aspx?" + strRul);
}
