$(document).ready(function() {
    ...
    var list = document.getElementById("HapSourceUC_ddlCodeType");
    ...
    list.options[1] = new Option("无来源", "0"); 
...
function fnGetSearch() {
    ...
    var Seller = $("#HapSourceUC_ddlCodeType").val();          //业务员ID（对应员工表ID）

    strUrl = 'Seller=' + escape(Seller) + '&FindDate=' + escape(FindDate) + '&FindDate1=' + escape(FindDate1);
...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/SellManager/SellChanceRepOfHapSource.ashx', //目标地址
...
function PrintSubjectsTotal() {
    ...
    window.open("PrintSellChanceRepOfHapSource.aspx?" + strRul);
}
