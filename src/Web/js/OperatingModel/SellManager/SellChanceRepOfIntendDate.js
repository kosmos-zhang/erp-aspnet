...
function fnGetSearch() {
    ...
    var strUrl = "";
    var IntendDate = $.trim($("#ddlStartDate").val()) + '-' + $.trim($("#ddlStartMonth").val()); //开始日期
    var IntendDate1 = $.trim($("#ddlEndDate").val()) + '-' + $.trim($("#ddlEndMonth").val()); //截止日期

    strUrl = '&IntendDate=' + escape(IntendDate) + '&IntendDate1=' + escape(IntendDate1);
...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/SellManager/SellChanceRepOfIntendDate.ashx', //目标地址
            ...
            if (msg != null) {
                if (msg.data.length != 0) {
                    ...
                    $.each(msg.data, function(i, item) {
                        if (item.IntendDate != null && item.IntendDate != "") {
                            $("<tr class='newrow'></tr>").append(
                         "<td height='22' align='center'>" + item.IntendDate + "</td>"
...
function PrintSubjectsTotal() {
    ...
    window.open("PrintSellChanceRepOfIntendDate.aspx?" + strRul);
}
