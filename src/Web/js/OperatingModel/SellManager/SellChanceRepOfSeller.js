...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/SellManager/SellChanceRepOfSeller.ashx', //目标地址
...
function PrintSubjectsTotal() {
    ...
    window.open("PrintSellChanceRepOfSeller.aspx?" + strRul);
}
...
function CheckInput() {
    ...
    var isFlag = true;
    var msgText = "";
    var Seller = $.trim($("#UserSeller").val()); //业务员ID（对应员工表ID）
    var FindDate = $.trim($("#txtStartDate").val()); //发现日期
    var FindDate1 = $.trim($("#txtEndDate").val()); //发现日期
