...
{//检索
    ...
    var CurrencyType=document.getElementById('MoneyType').title;
    ...
    var MyData = "pageIndex="+pageIndex+"&pageCount="+pageCount+"&action="+action+"&orderBy="+orderBy+
                     "&BeginTime="+escape(BeginTime)+"&EndTime="+escape(EndTime)+"&CurrencyType="+escape(CurrencyType)+"&CurrencyTypeName="+escape(CurrencyTypeName); 
    var search = "";
    search+="&BeginTime="+BeginTime;//
    search+="&EndTime="+EndTime;//
    search+="&orderBy="+orderBy;
    search+="&CurrencyTypeName="+CurrencyTypeName;
    search+="&CurrencyType="+CurrencyType;
    search+="";
    ...
    $.ajax({
           ...
           url:  '../../../Handler/OperatingModel/SellManager/SellProductTypeReport.ashx',//目标地址
           cache:false,
           data: MyData,//数据
           ...
           success: function(msg){
                    ...
                    var j =1;
                      $.each(msg.data,function(i,item){
                        if(item.TotalTax != null && item.TotalTax != "")
                        $("<tr class='newrow'></tr>").append("<td height='22' align='center'>"+item.CodeName+"</td>"+
                        "<td height='22' align='center' style='display:none'>"+(j++)+"</td>"+                                                
                        "<td height='22' align='right'>" +item.ProductCount+ "</td>"+
                        ...
                        "<td height='22' align='right'>"+parseFloat(item.TotalTax).toFixed(2) +"</td>"+
                        "<td height='22' align='center'>"+parseFloat((item.SellTax/item.AllTotalFee)*100).toFixed(2)+"%"+"</td>").appendTo($("#tblDetailInfo tbody"));                                              
                   });
...
//                           +"<td height='22' align='center'>"+msg.TotalTax+"</td><td height='22' align='center'></td>").appendTo($("#tblDetailInfo tbody")); 
//                             //页码
            ShowPageBar(
...
function PrintSpreadsheetBalanceTotal()
{
    ...
    window.open("PrintSellProductTypeReport.aspx?type=1"+searchStr);
}
function GetFloat(a)
{
   if(a!=null && a!='')
