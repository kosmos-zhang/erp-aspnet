...
{//检索
    //检索条件
     ...
     var HiddenProcessWay=requestQuaobj['HiddenProcessWay'];
     currentPageIndex = pageIndex;
      
    var MyData = "pageIndex="+pageIndex+"&pageCount="+pageCount+"&ProductID="+ProductID+"&BeginDate="+BeginDate+"&orderby="+orderBy+"&EndDate="+EndDate+"&HiddenProcessWay="+HiddenProcessWay;             
                 
   
    $.ajax({
           type: "POST",//用POST方式传输
           dataType:"json",//数据格式:JSON
           url:  '../../../Handler/OperatingModel/StorageManager/GetNoPassDetail.ashx',//目标地址
           cache:false,
            data: MyData,//数据
           ...
           success: function(msg){
                    ...
                    $.each(msg.data,function(i,item){
                        if(item.NotPassNum != null && item.NotPassNum != ""){
                        var testProductName=item.ProductName;
                        isSearch=1;
                        if(testProductName!=null)
                        {
                            if(testProductName.length>25)
                            {
                                testProductName=testProductName.substring(0,25)+'...';
                            }
                        }
                        $("<tr class='newrow'></tr>").append(
                        "<td height='22' align='center' style='display:none'>"+(j++)+"</td>"+
                        "<td height='22' align='center'>"+testProductName +"</td>"+
                        "<td height='22' align='center'><span >" + item.Specification + "</td>"+
                        "<td height='22' align='center'>"+ item.CodeName +"</a></td>"+
                        "<td height='22' align='center'>"+ item.ProcessWayName +"</a></td>"+
                        "<td height='22' align='center'>"+item.NotPassNum +"</a></td>").appendTo($("#pageDataList1 tbody"));}
                   });
                    //页码
