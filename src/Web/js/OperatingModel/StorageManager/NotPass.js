...
function TurnToPage(pageIndex)
{
    ...
    var BeginTime=document.getElementById('txtStartDate').value;  
    ...
    var ReasonID=document.getElementById('ddlReason').value;

    currentPageIndex = pageIndex;      
    var MyData = "pageIndex="+pageIndex+"&pageCount="+pageCount+"&action="+action+"&orderBy="+orderBy+
                     "&BeginTime="+escape(BeginTime)+"&EndTime="+escape(EndTime)+"&ReasonID="+escape(ReasonID);                   
    var search = "";
    search+="&BeginTime="+BeginTime;//
    search+="&EndTime="+EndTime;//
    search+="&ReasonID="+ReasonID;//
    ...
    $.ajax({
           ...
           url:  '../../../Handler/OperatingModel/StorageManager/NotPass.ashx',//目标地址
           cache:false,
           data: MyData,//数据
           ...
           success: function(msg){
                    ...
                    var j =1;
                     $.each(msg.data,function(i,item){
                        ...
                        "<td height='22' align='center'>"+ item.ProcessWay +"</td>"+
...
function PrintSpreadsheetBalanceTotal()
{
    ...
    window.open("PrintNotPass.aspx?type=1"+searchStr);
}
function GetFloat(a)
{
   if(a!=null && a!='')
