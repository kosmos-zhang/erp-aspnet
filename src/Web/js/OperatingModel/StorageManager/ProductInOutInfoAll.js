...
function DoSearch(currPage)
{
 ...
 /*-----------------------------------------DoSearch结束----------------------------------------------*/
    ...
    function TurnToPage0(pageIndex)
    {
           ...
           url:  '../../../Handler/OperatingModel/StorageManager/ProductInOutInfoAll.ashx',//目标地址
           data:UrlParam,
           cache:false,
                    ...
                    if (msg.data.length != 0)
                    {
                        ...
                        $.each(msg.data,function(i,item){
                            if(item.ProductNo != null && item.ProductNo != "")
                            {
                                ...
                                "<td height='22' align='center'>"+NumRound(item.OutCountTotal,2)+"</td>"+
                                // "<td height='22' align='center'>"+NumRound(item.OutPriceTotal,2)+"</td>"+
                                "<td height='22' align='center'>"+NumRound(item.InCountTotal,2)+"</td>").appendTo($("#pageDataList1 tbody"));
...
function fnPrint() {
    ...
    window.open("PrintProductInOutAll.aspx?order="+order+"&" + strRul);
}
...
//转向时带上条件---To销售
function fnGoInOutSell()
{
    var UrlParam="txtProductNo="+escape($("#txtProductNo").val())+
   "&txtProductName="+escape($("#txtProductName").val())+
   "&txtStartDate="+escape($("#txtStartDate").val())+
   "&txtEndDate="+escape($("#txtEndDate").val());
    window.location.href= "ProductInOutInfoSell.aspx?"+ UrlParam;
}