...
function DoSearch(currPage)
{
   ...
   var isFlag = true;
           ...
           var ddlStorageID=$("#ddlStorageID").val();
           ...
           "&ddlStorageID="+escape(ddlStorageID);
 ...
 /*-----------------------------------------DoSearch结束----------------------------------------------*/
    ...
    function TurnToPage(pageIndex)
    {
           ...
           url:  '../../../Handler/OperatingModel/StorageManager/ProductStockTotalPrice.ashx',//目标地址
           cache:false,
           data:UrlParam,
                    ...
                    if (msg.data.length != 0)
                    {
                        ...
                        $.each(msg.data,function(i,item){
                            if(item.ProductNo != null && item.ProductNo != "")
                            {
                                ...
                                "<td height='22' align='center'>"+NumRound(item.UnitPrice,2)+"</td>"+
                                "<td height='22' align='center'>"+NumRound(item.TotalPrice,2)+"</td>"+
                                "<td height='22' align='center'>"+NumRound(item.TaxRate,2)+"</td>"+
                                "<td height='22' align='center'>"+NumRound(item.TaxPrice,2)+"</td>"+
                                "<td height='22' align='center'>"+NumRound(item.TaxUnitPrice,2)+"</td>"+
                                "<td height='22' align='center'>"+NumRound(item.TaxTotalPrice,2)+"</td>").appendTo($("#pageDataList1 tbody"));
...
function fnPrint() {
    ...
    window.open("PrintProductStockTotalPrice1.aspx?order="+order+"&" + strRul);
}