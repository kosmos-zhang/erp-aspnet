...
function SearchPurNoPassInfo(pageIndex)
{
    ...
    var URLParams = "pageIndex="+pageIndex+"&pageCount="+pageCount+"&action="+action+"&orderBy="+orderBy+
                     "&BeginDate="+escape(BeginDate)+"&EndDate="+escape(EndDate)+"&CustID="+escape(CustID);   
    var search = "";
    search+="&BeginDate="+BeginDate;//开始日期
    search+="&EndDate="+EndDate;//结束日期
    search+="&CustID="+CustID;//供应商
    ...
    $.ajax({
           ...
           url:  '../../../Handler/OperatingModel/StorageManager/PurchaseApplyNoPass.ashx?'+URLParams,//目标地址
           cache:false,
...
//                   document.getElementById('TotalNotPassCount1').innerHTML=msg.TotalNoPassCount;
            ...
            ShowPageBar(
                ...
                "<%= Request.Url.AbsolutePath %>",//[url]
                {
                    ...
                    First:"首页",
                    End:"末页",
                    onclick:"SearchPurNoPassInfo({pageindex});return false;"
                }
            );
...
function ChangePageCountIndex(newPageCount, newPageIndex)
{
    ...
    else
    {
        ...
        //显示页面数据
        SearchPurNoPassInfo(parseInt(newPageIndex));
    }
}
/*
* 排序处理
