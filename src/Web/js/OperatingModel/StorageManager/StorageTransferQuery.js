...
function TurnToPage(pageIndex)
{
        if(cTrim(document.getElementById("txtTransferNo").value,0)!="")
        {
            ...
            {
                popMsgObj.Show("调拨单列表|","调拨单编号中含有特殊字符|");
                return;
            }
        }
        if(cTrim(document.getElementById("txtTitle").value,0)!="")
        {
            ...
            {
                popMsgObj.Show("调拨单列表|","调拨单主题中含有特殊字符|");
                return;
            }
        }
           ...
           {    //alert(msg.data);
                    ...
                    $.each(msg.data,function(i,item){
                        ...
                        $("<tr class='newrow'></tr>").append(
                            ...
                            "<td align=\"center\" ><a href=\"../../office/StorageManager/StorageTransferSave.aspx?ModuleID="+document.getElementById("MoudleID").value+"&action=EDIT&TransferID="+item.ID+"&TransferNo="+item.TransferNo+getFlowStatus(item.FlowStatus)+"\">"+item.TransferNo+"</a></td>"+
                             ...
                             "<td align=\"center\" >"+item.TransferCount +"</td>"+
                             "<td align=\"center\" >"+item.TransferPrice +"</td>"+
...
//        return "<input type=\"checkbox\" id=\"chk_list_"+item.ID+"\" name=\"chk\" value=\""+item.ID+"\"  disabled=\"true\"  onclick=\"unSelectAll('"+item.ID+"');\"/></td>"
 ...
 function setStatusAction(item)
 {
    if(item.BillStatus!="制单")
    {
        ...
        {
            return "<a href=\"../../office/StorageManager/StorageTransferOut.aspx?ModuleID="+document.getElementById("ModuleInOutID").value+"&TransferID="+item.ID+"\">"+item.BusiStatus+"</a>";
        }
        else if(item.BusiStatus=="调拨入库")
        {
            return "<a href=\"../../office/StorageManager/StorageTransferIn.aspx?ModuleID="+document.getElementById("ModuleInOutID").value+"&TransferID="+item.ID+"\">"+item.BusiStatus+"</a>";
        }
        else
