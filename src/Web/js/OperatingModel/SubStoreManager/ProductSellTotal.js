...
function fnGetSearch() {
    ...
    strUrl = 'SellDate=' + escape(SellDate) + '&DeptID=' + escape(DeptID)+'&SellEndDate=' + escape(SellEndDate)+'&ProductID=' + escape(ProductID);
...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/SubStoreManager/ProductSellTotal.ashx', //目标地址
        ...
        success: function(msg) {
            ...
            if (msg != null) {
                if (msg.data.length != 0) {
                    ...
                    $.each(msg.data, function(i, item) { if(item.ProdNo != null && item.ProdNo != ""){
//                    var SellTotalFee=parseFloat(item.SellTotalFee);
//                    var BackTotalFee=parseFloat(item.BackTotalFee);
                        $("<tr class='newrow'></tr>").append("<td height='22' align='center'>" + item.ProdNo + "</td>"
                        ...
                        + "<td height='22' align='center'>" + item.Specification + "</td>"
                           + "<td height='22' align='center'>" + item.CodeName + "</td>"
                              + "<td height='22' align='center'>" + FormatAfterDotNumber(item.SellCount,selPoint) + "</td>"
                                 + "<td height='22' align='center'>"+FormatAfterDotNumber(item.SellTotalFee,selPoint) + "</td>"
                                  + "<td height='22' align='center'>" + FormatAfterDotNumber(item.BackCount,selPoint) + "</td>"
                                   + "<td height='22' align='center'>" + FormatAfterDotNumber(item.BackTotalFee,selPoint) + "</td>"
...
function PrintSubjectsTotal() {
 ...
 if(ordering=="a")
    ...
    window.open("PrintOfficeProductSellTotal.aspx?orderby="+orderBy+"&ordering="+ordering+"&" + strRul);
...
//填充物品
 function Fun_FillParent_Content(id,no,productname)
{
    document.getElementById("txt_prod").value=productname;
    document.getElementById("hf_ProductID").value=id;
}
function ShowList()
{
var Codition =document.getElementById("hd_condiction").value;

popTechObj.ShowList('txt_prod','hf_ProductID',Codition)
}
