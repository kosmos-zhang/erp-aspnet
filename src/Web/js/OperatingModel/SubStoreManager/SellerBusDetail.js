...
function fnGetSearch() {
    ...
    strUrl = 'SellDate=' + escape(SellDate) + '&DeptID=' + escape(DeptID)+'&SellEndDate=' + escape(SellEndDate)+'&UserSellerName=' + escape(UserSellerName)+'&DeptName=' + escape(DeptName);
...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/SubStoreManager/SellerBusDetail.ashx', //目标地址
            ...
            if (msg != null) {
                if (msg.data.length != 0) {
                    ...
                    $.each(msg.data, function(i, item) {
                      if(item.Seller != null && item.Seller != ""){
                    var Fee=parseFloat(item.RealTotal);var temp='';if(flag!='0')temp="<td height='22' align='center'>" + item.DeptID + "</td>";
                        $("<tr class='newrow'></tr>").append(""+temp+""
                        + "<td height='22' align='center'>" + item.Seller + "</td>"
                          + "<td height='22' align='center'>" + item.OrderDate.substring(0,10) + "</td>"
                           + "<td height='22' align='center'>" + item.OrderNo + "</td>"
                            + "<td height='22' align='center'>" + item.isOpenbill + "</td>"
...
function PrintSubjectsTotal() {
    ...
    var strRul = $("#hiddSearch").val();
     var flag=$("#hf_flag").val();
       if(flag=="0")
   {     window.open("PrintOfficeSellerBusDetail.aspx?orderby="+orderBy+"&ordering="+ordering+"&" + strRul);}
   else if(flag=="1")
     { window.open("PrintSellerSellDetail.aspx?orderby="+orderBy+"&ordering="+ordering+"&" + strRul);}
}
