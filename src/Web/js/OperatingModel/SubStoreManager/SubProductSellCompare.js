...
function fnGetSearch() {
    ...
    var SellDate = $.trim($("#txtStartDate").val()); //销售日期
      var SellEmdDate = $.trim($("#txtEndDate").val()); //销售日期
    var ProductID = $.trim($("#hf_ProductID").val()); //产品名称
    strUrl = 'SellDate=' + escape(SellDate) + '&ProductID=' + escape(ProductID)+'&SellEmdDate=' + escape(SellEmdDate);
...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/SubStoreManager/SubSellProductCompare.ashx', //目标地址
                    ...
                    $.each(msg.data, function(i, item) {
                     if(item.DeptName != null && item.DeptName != ""){
                        ...
                        + "<td height='22' align='center'>" + item.ProductCount + "</td>"
                           + "<td height='22' align='center'>" + item.TotalFee + "</td>"
...
function PrintSubjectsTotal() {
 if(ordering=="a")
    ...
    window.open("PrintSubProductSellCompare.aspx?orderby="+orderBy+"&ordering="+ordering+"&" + strRul);
