...
function fnGetSearch() {
    ...
    strUrl = 'SellDate=' + escape(SellDate) + '&DeptID=' + escape(DeptID) + '&SellEndDate=' + escape(SellEndDate);
...
function TurnToPage(pageIndex) {
    ...
    //设置当前页
    var serch = document.getElementById("hiddSearch").value;

    var strUrl = serch + '&' + '&pageIndex=' + pageIndex + '&pageCount=' + pageCount + '&orderby=' + escape(orderBy);
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/SubStoreManager/SubSellTotal.ashx', //目标地址
            ...
            if (msg != null) {
                if (msg.data.length != 0) {
                    ...
                    $.each(msg.data, function(i, item) {
                        ...
                        + "<td height='22' align='center'>" + FormatAfterDotNumber(item.CountTotal, selPoint) + "</td>"
                           + "<td height='22' align='center'>" + FormatAfterDotNumber(item.RealTotal, selPoint) + "</td>"
                              + "<td height='22' align='center'>" + FormatAfterDotNumber(item.BackCountTotal, selPoint) + "</td>"
                                 + "<td height='22' align='center'>" + FormatAfterDotNumber(item.BackRealTota, selPoint) + "</td>"
...
function PrintSubjectsTotal() {
    ...
    window.open("PrintSubSellTotal.aspx?orderby=" + orderBy + "&ordering=" + ordering + "&" + strRul);
