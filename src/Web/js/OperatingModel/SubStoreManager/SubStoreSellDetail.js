...
function fnGetSearch() {
    ...
    strUrl = 'SellDate=' + escape(SellDate) + '&DeptID=' + escape(DeptID)+'&SellEndDate=' + escape(SellEndDate)+'&DeptName=' + escape(DeptName);
...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        url: '../../../Handler/OperatingModel/SubStoreManager/SubStoreSellDetail.ashx', //目标地址
        ...
        success: function(msg) {
            ...
            if (msg != null) {
                if (msg.data.length != 0) {
                    ...
                    {
                    var Fee=parseFloat(item.SellTotalFee);
                                  ...
                                  + "<td height='22' align='center'>" + item.ProductName + "</td>"
                                   + "<td height='22' align='center'>" + item.Specification + "</td>"
                                    + "<td height='22' align='center'>" + item.CodeName + "</td>"
                                     + "<td height='22' align='right'>" +FormatAfterDotNumber( item.ProductCount,selPoint) + "</td>"
                                      + "<td height='22' align='right'>" +FormatAfterDotNumber( Fee,selPoint) + "</td>"
...
if(ordering=="a")
    ...
    window.open("PrintSubStorageSellDetail.aspx?orderby="+orderBy+"&ordering="+ordering+"&" + strRul);
}
