           ...
           success: function(msg){
                    ...
                    $.each(msg.data,function(i,item){
                      //if(item.ID != null && item.ID != "")
                        ...
                        "<input id='Check1' name='Check1' onclick=getEmployName('"+item.id+"','"+item.EmployeeName+"') value='选择'  type='radio'/>"+"</td>"+                       
...
//使调用页面获得员工ID、姓名
function getEmployName(id,employeename)
{
    document.getElementById(hfEmployeeID).value = id;
...
{
    document.getElementById(txtEmployeeName).onclick = ShowEmployee;
}