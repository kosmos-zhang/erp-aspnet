...
function FistSearchCustAdvice()
{
      document.getElementById('isreturn').value='1';
      SearchCustAdviceData(1);
...
function SearchCustAdviceData(pageIndex)
{//检索
    //检索条件
    if(CheckInput())
    {   
    ...
    var txtAdvicetNo =document.getElementById("txtAdvicetNo").value;  //单据编号//
    var txtTitle = document.getElementById("txtTitle").value; //   
    var CustID = document.getElementById("hfCustID").value; //提出建议客户ID
    
    var DestClerk=document.getElementById('hiddenExecutor').value;     //接待人
    ...
    var MaxCount=document.getElementById('Text2').value;
    ...
    var MyData = "pageIndex="+pageIndex+"&pageCount="+pageCount+"&action="+action+"&orderBy="+orderBy+
                     "&txtAdvicetNo="+escape(txtAdvicetNo)+"&txtTitle="+escape(txtTitle)+"&CustID="+escape(CustID)+"&DestClerk="+DestClerk+"&txtAdviceType="+txtAdviceType+
                     "&txtState="+escape(txtState)+"&BeginTime="+BeginTime+"&EndTime="+EndTime+"&MaxCount="+MaxCount;
                 
   
    $.ajax({
           type: "POST",//用POST方式传输
           dataType:"json",//数据格式:JSON
           url:  '../../../Handler/Office/CustManager/CustAdviceList.ashx',//目标地址
           cache:false,
            data: MyData,//数据
           ...
           success: function(msg){
                   ...
                   // alert(msg.totalCount);
                    if(parseFloat(document.getElementById('ShowPageCount').value)>msg.totalCount)
                    {
                    ...
                    $.each(msg.data,function(i,item){
                        ...
                        "<td height='22' align='center'><a href='CustAdviceAdd.aspx?ModuleID=2021901&myAction=edit&ID="+item.ID+"' title=\""+item.AdviceNo+"\">"+testApplyNo +"</a></td>"+
                        "<td height='22' align='center'><span title=\""+item.Title+"\">" + testTitle + "</td>"+
                        "<td height='22' align='center'>"+ item.CustName +"</a></td>"+
                        "<td height='22' align='center'>"+ item.LinkManName +"</a></td>"+
                        "<td height='22' align='center'>"+ item.EmployeeName +"</a></td>"+
                        "<td height='22' align='center'>"+ item.Accept+"</a></td>"+
                        "<td height='22' align='center'>"+ item.AdviceDate +"</a></td>"+
                        "<td height='22' align='center'>"+ item.AdviceType +"</a></td>"+
                        "<td height='22' align='center'>"+ item.State +"</a></td>").appendTo($("#pageDataList1 tbody"));}
                   });
                    //页码
                   ...
                   "<%= Request.Url.AbsolutePath %>",//[url]
                    ...
                    onclick:"SearchCustAdviceData({pageindex});return false;"}//[attr]
                    );
...
function CheckInput()
{
    ...
    var ReportNo=document.getElementById('txtAdvicetNo').value;
    ...
    function ChangePageCountIndex(newPageCount,newPageIndex)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                SearchCustAdviceData(parseInt(newPageIndex));
            }
        }
    }
    //排序
    ...
    {   
        ...
        var UrlParam = "myAction="+Action+"&strID="+OptionCheckArray.toString();
            ...
            $.ajax({ 
                  ...
                  error: function() 
                  {
                    popMsgObj.ShowMsg('删除客户建议时请求发生错误');
