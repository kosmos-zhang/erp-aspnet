$(document).ready(function()
{
    ...
    { 
        ...
        $("#ddlApply").val(requestobj['EmplApplyNo']);
        //申请日期
        $("#UserEmployeeName").val(requestobj['UserEmployeeName']);
...
function TurnToPage(pageIndex)
{
    ...
    $.ajax({
        type: "POST",//用POST方式传输
        url:  '../../../Handler/Office/HumanManager/EmployeeShift_Query.ashx?' + postParam,//目标地址
            ...
            $.each(msg.data
                ,function(i,item)
                {
                    if(item.ID != null && item.ID != "")
                    {
                        ...
                        var StrTitle=item.Title;var No=item.NotifyNo;
                        ...
                        $("<tr class='newrow'></tr>").append("<td height='22' align='center'><input type='hidden' id='txtStatus_" + item.NotifyNo 
                            ...
                            + "<td height='22' align='center'>" + item.EmplApplyNo + "</td>" //对应调职申请单
                            + "<td height='22' align='center'>" + item.EmployeeNo + "</td>" //员工编号
                            + "<td height='22' align='center'>" + item.EmployeeName + "</td>" //员工姓名
                            + "<td height='22' align='center'>" + item.NowDeptName + "</td>" //原部门
                            + "<td height='22' align='center'>" + item.NowQuarterName + "</td>"  //原岗位 
                            + "<td height='22' align='center'>" + item.OutDate + "</td>"  //调出时间 
                            + "<td height='22' align='center'>" + item.NewDeptName + "</td>" //调至部门
                            + "<td height='22' align='center'>" + item.NewQuarterName + "</td>" //调入岗位
...
function DoDelete()
{
    ...
    else
    {
        ...
        { 
            type: "POST",
            url: "../../../Handler/Office/HumanManager/EmployeeShift_Query.ashx?" + postParam,
...
function GetLinkParam()
{
    ...
    linkParam = "EmployeeShift_Edit.aspx?FromPage=1&ModuleID=" + ModuleID 
