...
function DoSearch(currPage)
{
    var search = "";
    //申请编号
    search += "MoveApplyNo=" + document.getElementById("txtMoveApplyNo").value;
    //申请人
...
{
    //申请编号
    document.getElementById("txtMoveApplyNo").value = "";
    //申请人
...
function TurnToPage(pageIndex)
{
    ...
    $.ajax({
        type: "POST",//用POST方式传输
        url:  '../../../Handler/Office/HumanManager/MoveApply_Query.ashx?' + postParam,//目标地址
                ...
                ,function(i,item)
                {
                    if(item.ID != null && item.ID != "")
                    {
                        ...
                        $("<tr class='newrow'></tr>").append("<td height='22' align='center'>"
                            + "<input id='chkSelect' name='chkSelect' value='" + item.MoveApplyNo + "'  type='checkbox' onclick=\"SetCheckAll(this, 'chkSelect', 'chkCheckAll');\"  />" + "</td>" //选择框
                            + "<td height='22' align='center'><a href='" + GetLinkParam() + "&ID=" + item.ID + "&StatusName="+StatusName+"')>" + item.MoveApplyNo + "</a></td>" //申请编号
                            + "<td height='22' align='center'>" + item.EmployeeName + "</td>" //申请人
                            + "<td height='22' align='center'>" + item.ApplyDate + "</td>" //申请时间
                            + "<td height='22' align='center'>" + item.DeptName + "</td>" //目前部门
                            + "<td height='22' align='center'>" + item.QuarterName + "</td>" //目前岗位
                            + "<td height='22' align='center'>" + item.HopeDate + "</td>"  //希望离职日期 
...
function DoDelete()
{
    ...
    else
    {
        ...
        { 
            type: "POST",
            url: "../../../Handler/Office/HumanManager/MoveApply_Query.ashx?" + postParam,
...
function GetLinkParam()
{
    ...
    linkParam = "MoveApply_Edit.aspx?ModuleID=" + ModuleID 
