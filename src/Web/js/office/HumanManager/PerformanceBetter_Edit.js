...
//    ModuleID=2011808&hidIsliebiao=1&PageIndex=1&PageCount=10&searchPlanNo=&Title=&StartDate=&EndDate
//=&EmployeeID=&Flag=1&PlanNo=001

          requestobj = GetRequest(); 
    var searchPlanNo = requestobj['searchPlanNo'];
    var Title = requestobj['Title'];
    var StartDate = requestobj['StartDate'];
    var EndDate = requestobj['EndDate'];
    var EmployeeID = requestobj['EmployeeID']; 
    
    var UserApplyUserName = requestobj['UserApplyUserName']; 
    if(typeof(searchPlanNo)!="undefined")
    { 
       $("#txtSearchTask").attr("value",searchPlanNo);//客户简称
       $("#inptSearchTitle").attr("value",Title);
       $("#txtSearchScoreEmployee").attr("value",EmployeeID);
       
       $("#txtStartDate").attr("value",StartDate);
       $("#txtEndDate").attr("value",EndDate);
...
{
document.getElementById("dvSearchTaslNum").style.diplay="none";
document.getElementById("selSearchTaskNum").options.length=0;
...
{ 
    var search = "";
    //要素名称
    search += "searchPlanNo=" +escape ( document.getElementById("txtSearchTask").value.Trim());
    //启用状态
    search += "&Title=" +escape ( document.getElementById("inptSearchTitle").value.Trim());
     search += "&StartDate=" + document.getElementById("txtStartDate").value.Trim();
       search += "&EndDate=" + document.getElementById("txtEndDate").value.Trim();
          search += "&EmployeeID=" +escape ( document.getElementById("txtSearchScoreEmployee").value.Trim());
...
function GetLinkParam()
{
    ...
    linkParam = "PerformanceBetter.aspx?ModuleID=" + ModuleID 
...
function TurnToPage(pageIndex)
{
    ...
    $.ajax({
        type: "POST",//用POST方式传输
        url:  '../../../Handler/Office/HumanManager/PerformanceBetter.ashx?' + postParam,//目标地址
            ...
            $.each(msg.data
                ,function(i,item)
                {
                    ...
                    $("<tr class='newrow'></tr>").append( "<td height='22' align='center'>"
                        + "<input id='chkSelect' name='chkSelect' value='" + item.PlanNo+ "'  type='checkbox' class='chkSelectPlanNo' onpropertychange='getChage(this)' />" + "</td>" //选择框
                        +"<td height='22' align='center'>" + item.PlanNo + "</td>"//考核任务编号
                        + "<td height='22' align='center'><a href='"+GetLinkParam()+"&PlanNo="+item.PlanNo+"' >" + item.Title + "</a></td>" //考核任务主题
	...
	function DoDelete()
{
  if(confirm("删除后不可恢复，确认删除吗！"))
	      {
var no;
var checkPlanNo=new Array ();
var checkNo=$(".chkSelectPlanNo");
var checkList=checkNo .length;
var sign=false ;
 for (var a=0;a<checkList ;a++)//取消所有的勾选项
 {
          //alert (tmList[a].value); 
          // alert (employeeID[i]);
    if ( checkNo[a] .checked==true)
    {
        sign=true ;
      }
  ...
  if (sign ==false )
  {
        showPopup("../../../Images/Pic/Close.gif","../../../Images/Pic/note.gif","请选择一项计划！");
        return;
  }
 for (var a=0;a<checkList ;a++)//取消所有的勾选项
 {
    ...
    if ( checkNo[a] .checked==true)
    {   
       //  alert (checkNo[a].value);
       checkPlanNo .push (checkNo[a].value);
        //    no=checkNo[a].value;
      }
...
//alert (no);
    ...
    $.ajax({ 
        type: "POST",
        url: "../../../Handler/Office/HumanManager/PerformanceBetter.ashx?action=DeletePlanInfo&PlanNoList=" + checkPlanNo,
        dataType:'json',
        cache:false,
        beforeSend:function()
        {
...
function DoBack()
{
 self.location='PerformanceBetter.aspx?ModuleID=2011808'; 
}
