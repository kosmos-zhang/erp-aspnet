...
function DoSearch(currPage)
{
 ...
 /*-----------------------------------------上面是查询-----------------------------------------------------------*/     
    ...
    function TurnToPage(pageIndex)
    {
           ...
           url:  '../../../Handler/Office/HumanManager/PerformanceRoyaltyBase.ashx?'+UrlParam,//目标地址
           cache:false,
           ...
           success: function(msg){
                    ...
                    $.each(msg.data,function(i,item){
                        ...
                        "<td height='22' align='center'><a style=\"cursor:pointer\" onclick=ShowNewDiv(\""+item.ID+"\",\""+item.EmployeeID+"\",\""+item.EmployeeName+"\",\""+item.TaskFlag+"\",\""+item.BaseMoney+"\")>查看</a></td>"+
                        ...
                        "<td height='22' align='center'>"+item.TaskFlagName+"</td>").appendTo($("#pageDataList1 tbody"));
      ...
      var ck = document.getElementsByName("Checkbox1");
            ...
            else
            {
                ...
                var UrlParam = "Act="+Act+"&strID="+IDArray;
                    $.ajax({ 
                          type: "POST",
                          url: "../../../Handler/Office/HumanManager/PerformanceRoyaltyBase.ashx?"+UrlParam,
...
function DoSave()
{
    ...
    var EmployeeID = $("#txtEmployeeID_uc").val();
    var BaseMoney = $("#txtBaseMoney").val();
    var Taskflag_uc = $("#sltTaskflag_uc").val();
    ...
    if (EmployeeID == "")
    {
        EmployeeID="0";//如果没有选择，那么赋为“0”，也就是默认
    }
    if (BaseMoney == "")
    {
        isFlag = false;
    ...
    var UrlParam = "&ID="+escape(ID) +"&EmployeeID="+escape(EmployeeID)
                    +"&BaseMoney="+escape(BaseMoney)
                    +"&Taskflag="+escape(Taskflag_uc)
                    +"&Act=edit";
...
//打开编辑层
function ShowNewDiv(ID,EmployeeID,EmployeeName,TaskFlag,BaseMoney)
{
    if(typeof(ID)!="undefined")
    {
        $("#txtIndentityID").val(ID);
        $("#txtEmployeeID_uc").val(EmployeeID);
        $("#UserEmployee_uc").val(EmployeeName);
        $("#txtBaseMoney").val(BaseMoney);
        document.getElementById("sltTaskFlag_uc").value=TaskFlag;
...
function New()
{
    ...
    $("#txtEmployeeID_uc").val("");
    $("#UserEmployee_uc").val("");
    $("#txtBaseMoney").val("");
    document.getElementById("sltTaskflag_uc").value="";
}
