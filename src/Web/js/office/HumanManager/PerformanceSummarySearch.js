    ...
    search += "&Title=" +escape (  document.getElementById("inptSearchTitle").value.Trim());
     search += "&TaskFlag=" + document.getElementById("selSearchTaskFlag").value.Trim();
       search += "&taskNum=" + document.getElementById("selSearchTaskNum").value.Trim();
         search += "&taskStatus=" + document.getElementById("selTaskStatus").value.Trim();
           search += "&employeeID=" + document.getElementById("txtSearchScoreEmployee").value.Trim();
...
function TurnToPage(pageIndex)
{
    ...
    //设置动作种类
    var action="SearchSurmmaryInfo";
    var postParam = "PageIndex=" + pageIndex + "&PageCount=" + pageCount + "&action=" + action + "&OrderBy=" + orderBy + "&" + searchCondition;
    //进行查询获取数据
    $.ajax({
        type: "POST",//用POST方式传输
        url:  '../../../Handler/Office/HumanManager/PerformanceSummary.ashx?' + postParam,//目标地址
        ...
        success: function(msg)
        {
            ...
            $.each(msg.data
                ,function(i,item)
                {
                    ...
                    $("<tr class='newrow'></tr>").append( "<td height='22' align='center'><a href='PerformanceSummarySearch_Edit.aspx?ModuleID=2011805&TemplateNo="+item .CompleteDate+"&EmployeeId="+item .ModifiedDate+"&TaskNo=" + item.TaskNo + "' >" + item.Remark + "</a></td>"//考核任务编号
                            ...
                            + "<td height='22' align='center'>" + item.Creator +"</td>"
                             + "<td height='22' align='center'>" + item.StartDate +"</td>"
                              ...
                              + "<td height='22' align='center'>" + item.Summaryer +"</td>"
                                + "<td height='22' align='center'>" + item.SummaryDate.substring(0,10) +"</td>").appendTo($("#tblDetailInfo tbody")
                          //启用状态
                    );
            });
            //页码
