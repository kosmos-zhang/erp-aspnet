...
var isFlag=true ;
    ...
    var search = "";
    //模板编号
    search += "TemplateNo=" +escape (  document.getElementById("txtTemplateNo").value.Trim());
    //主题
    search += "&Title=" + escape ( document.getElementById("txtTitle").value.Trim());
    //岗位
    search += "&QuarterID=" + document.getElementById("ddlQuarter").value.Trim();
...
{
    //模板编号
    document.getElementById("txtTemplateNo").value = "";
    //主题
...
function TurnToPage(pageIndex)
{
    ...
    $.ajax({
        type: "POST",//用POST方式传输
        url:  '../../../Handler/Office/HumanManager/RectCheckTemplate_Query.ashx?' + postParam,//目标地址
        ...
        success: function(msg)
        {
            ...
            $.each(msg.data
                ,function(i,item)
                {
                    ...
                    {
                    //alert(SubValue(10,item.Title));
                    $("<tr class='newrow'></tr>").append("<td height='22' align='center'>"
                        + "<input id='chkSelect' name='chkSelect' value='" + item.TemplateNo + "' type='checkbox' onclick=\"SetCheckAll(this, 'chkSelect', 'chkCheckAll');\"  />" + "</td>" //选择框
                        + "<td height='22' align='center'><a href='" + GetLinkParam() + "&TemplateID=" + item.ID + "')>" + item.TemplateNo + "</a></td>" //模板编号
                        + "<td height='22' align='left'><font title='"+item.Title+"'>" + SubValue(30,item.Title) + "</font></td>" //主题
                        + "<td height='22' align='left'>" + item.QuarterName + "</td>").appendTo($("#tblDetailInfo tbody")//岗位
                    );
                    }
            });
            //页码
...
function DoDelete()
{
    ...
    else
    {
        ...
        var postParam = "Action=del&DeleteNOs=" + escape(deleteNos);
        //删除
        $.ajax(
        { 
            type: "POST",
            url: "../../../Handler/Office/HumanManager/RectCheckTemplate_Query.ashx?" + postParam,
...
function GetLinkParam()
{
    ...
    linkParam = "RectCheckTemplate_Edit.aspx?ModuleID=" + ModuleID 
