...
function DoAdd() {
    ...
    objTD.className = "tdColInputCenter";
    objTD.innerHTML = "<input type='text' id='txtDept_" + no + "' value='" + DeptName + "' class='tdinput' style='width:98%;' readonly='readonly' />"
    ...
    if (no == 1) {
        ...
        document.getElementById("txtDept_" + row).readOnly = "true";
    ...
    for (var row = count - 1; row > 0; row--) {
        ...
        if (chkControl.checked) {
            ...
            if (ss > 0) {
                ...
                if (ss != 1) {
                    //	document .getElementById ("txtMiniMoney_"+ss).readOnly=false;
                }
                document.getElementById("txtTaxPercent_" + ss).readOnly = false;
...
function CalculateTotalSalary(obj, row) {
    ...
    if (arr.length <= 2) {
        ...
        if (personTaxPercent == null || personTaxPercent == "") {
            ...
            if (!IsNumeric(personTaxPercent, 4, 2)) {
        ...
        if (row == 1) {
            document.getElementById("txtMiniMoney_" + row).value = 0;
        }
        else {
...
function DoSave() {
    ...
    $.ajax({
        type: "POST",
        url: "../../../Handler/Office/HumanManager/SalaryCompanyRoyaltySet.ashx",
        data: postParams,
...
function DoSearch() {
    //获取值value
    var DeptID = $("#hidtxtSubCom").val();
    //获取文字
    //var ById = document.getElementById("dplSubCompany")
    var DeptName = $("#txtSubCom").val();
    if (DeptID == "") {
