...
function DoAdd(){
	...
	objTD.innerHTML = "<input type='hidden' id='txtPieceworkID_" + no + "' value='' />"
	                + "<input type='hidden' id='txtEditFlag_" + no + "' value='0' />"
	                + "<input type='checkbox' onclick='SetCheckAll(this);' id='chkSelect_" + no + "'>";
	var eventClick = "";
	var disabled = "";
	//包含生产模块时
	if(document.getElementById("txtIsHaveProduct").value.Trim() == "1")
	{
	...
	objTD.className = "tdColInputCenter";
	objTD.innerHTML = "<input type='text' maxlength='50'" + eventClick + "style='width:98%;' class='tdinput' id='txtItemNo_" + no + "' />";
	//项目名称
	objTD = objTR.insertCell(-1);
	objTD.className = "tdColInputCenter";
	objTD.innerHTML = "<input type='text' maxlength='50'" + disabled + "style='width:98%;' class='tdinput' id='txtItemName_" + no + "' />";
	//单价
	objTD = objTR.insertCell(-1);
	objTD.className = "tdColInputCenter";
...
//      }
	...
	for (var row = count - 1; row > 0; row--)
	{
        ...
        { 
            type: "POST",
            url: "../../../Handler/Office/HumanManager/SalaryPiecework.ashx?" + postParam,
	    ...
	    document.getElementById("spanMsg").innerHTML = CreateErrorMsgDiv("错误|",  " 请求发生错误!|");
            }, 
...
function DoSave()
{
    ...
    $.ajax({ 
        type: "POST",
        url: "../../../Handler/Office/HumanManager/SalaryPiecework.ashx?" + postParams,
        dataType:'json',//返回json格式数据
        cache:false,
        beforeSend:function()
        {
            ...
            {                
                ...
                $.each(data.DataInfo
                    ,function(i,item)
                    {
	                    ...
	                    var objTD = objTR.insertCell(-1);
	                    objTD.className = "tdColInputCenter";
	                    objTD.innerHTML = "<input type='hidden' id='txtPieceworkID_" + no + "' value='" + item.ID + "' />"
...
{    
	...
	for (var i = 1 ; i < count ; i++)
	{
	    ...
	    if (table.rows[i].style.display != "none")
	    {
            ...
            salaryCount++;
	        //工资编号
            strParams += "&PieceworkID_" + salaryCount + "=" +escape (  document.getElementById("txtPieceworkID_" + i).value.Trim());
	        //编辑模式
