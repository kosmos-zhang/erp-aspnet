...
* 查询培训列表
*/
function SearchTrainingInfo(currPage)
{
    if(!CheckInput())
    {
        return;
    ...
    search += "&StartToDate=" + document.getElementById("txtAsseEndDate").value;
    //设置检索条件
...
function TurnToPage(pageIndex)
{
    ...
    $.ajax({
        type: "POST",//用POST方式传输
        url:  '../../../Handler/Office/HumanManager/Training_Query.ashx?' + postParam,//目标地址
            ...
            $.each(msg.data
                ,function(i,item)
                {
                    ...
                    $("<tr class='newrow'></tr>").append("<td height='22' align='center'>"
                        + "<input id='chkSelect' name='chkSelect'  value='" + item.TrainingNo + "'  type='checkbox' onclick=\"SetCheckAll(this, 'chkSelect', 'chkCheckAll');\" />" + "</td>" //选择框
                        + "<td height='22' align='center'><a title='" + item.TrainingNo + "' href='" + GetLinkParam() +"&TrainingID=" + item.ID + "')>" + SubValue(15,item.TrainingNo) + "</a></td>" //培训编号
                        ...
                        + "<td height='22' align='center' title='" + item.TrainingPlace + "'>" + SubValue(15,item.TrainingPlace) + "</td>").appendTo($("#tblDetailInfo tbody")//培训地点
                    );
            });
            //页码
            ShowPageBar(
...
function DeleteTrainingInfo()
{
    ...
    else
    {
        ...
        { 
            type: "POST",
            url: "../../../Handler/Office/HumanManager/Training_Query.ashx?" + postParam,
...
function ToTrainingAsse()
{
    ...
    var selectNo = chkValue.substring(0, chkValue.length - 1);
    selectLength = selectNo.split(",");
    ...
    {
        showPopup("../../../Images/Pic/Close.gif","../../../Images/Pic/note.gif","您最多只能选择一条记录！");
        return;
    }
    //获取模块功能ID
    var ModuleID = document.getElementById("hidModuleIDAsse").value;
    //获取查询条件
    searchCondition = document.getElementById("hidSearchCondition").value;
    //是否点击了查询标识
    var flag = "0";//默认为未点击查询的时候
    if (searchCondition != "") flag = "1";//设置了查询条件时
    window.location.href = "TrainingAsse_Edit.aspx?ModuleID=" + ModuleID 
                            + "&PageIndex=" + currentPageIndex + "&PageCount=" + pageCount + "&" + searchCondition 
                            + "&Flag=" + flag + "&From=1&SelectTrainingNo=" + selectNo;
}
