...
function fnPrint() {
    ...
    var StartConfirmDate = document.getElementById("txtStartConfirmDate").value.Trim();
    var EndConfirmDate = document.getElementById("txtEndConfirmDate").value.Trim();
    var dataSource = "";
    if (document.getElementById("radArrive").checked == true) {
        dataSource = "Arrive";
    }
    else {
        dataSource = "Order";
    ...
    window.open("PurchaseDetailsPrint.aspx?pageIndex=" + currentPageIndex + "&pageCount=" + pageCount + "&orderby=" + orderBy + "&ProviderID=" + escape(ProviderID) + "&dataSource=" + escape(dataSource) + "&ProductID=" + escape(ProductID) + "&StartConfirmDate=" + escape(StartConfirmDate) + "&EndConfirmDate=" + escape(EndConfirmDate) + "");
...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        ...
        url: '../../../Handler/office/PurchaseManager/PurchaseDetail.ashx', //目标地址
        cache: false,
        data: "pageIndex=" + pageIndex + "&pageCount=" + currentpageCount + "&orderby=" + orderBy + document.getElementById("hidSearchCondition").value,
        ...
        success: function(msg) {
            ...
            $.each(msg.data, function(i, item) {
                if (item.ProviderID != null && item.ProviderID != "") {
                    ...
                    if (ArriveNo != null) {
                        if (ArriveNo.length > 10) {
                            ArriveNo = ArriveNo.substring(0, 10) + '...';
                        }
                    }
                    ...
                    //                        "<td height='22' align='center'><a href='" + GetLinkParam(ProductID) +"&ProductID=" + item.ProductID + " ')>"+item.ProductNo+"</a></td>"+
                        ...
                        "<td height='22' align='center'>" + item.Specification + "</a></td>" +
                         "<td height='22' align='center'>" + item.ColorName + "</a></td>" +
                          ...
                          "<td height='22' align='center'>" + item.ProductCount + "</a></td>" +
                            "<td height='22' align='center'>" + item.UnitPrice + "</a></td>" +
                               "<td height='22' align='center'><span title=\"" + item.ProviderName + "\">" + item.ProviderName + "</a></td>" +
                                  "<td height='22' align='center'><span title=\"" + item.PurchaseName + "\">" + item.PurchaseName + "</a></td>" +
                                 "<td height='22' align='center'><span title=\"" + item.BillNo + "\">" + item.BillNo + "</a></td>"  ).appendTo($("#pageDataList1 tbody"));
                }
...
function fnCheck() {
    ...
    var msgText = "";
    var isFlag = true;
    if (document.getElementById("txtStartConfirmDate").value.Trim() == "") {
        isFlag = false;
        fieldText += "起始日期|";
        msgText += "起始日期不能为空|";
    }
    if (document.getElementById("txtEndConfirmDate").value.Trim() == "") {
        isFlag = false;
        fieldText += "结束日期|";
        msgText += "结束日期不能为空|";
    }
...
function SearchPurchaseArriveCollect() {
    ...
    var Isliebiao = 1;
    var URLParams = "&Isliebiao=" + escape(Isliebiao) + "&ProviderID=" + escape(ProviderID) + "&ProductID=" + escape(ProductID) + "&StartConfirmDate=" + escape(StartConfirmDate) + "&dataSource=" + escape(dataSource) + "&EndConfirmDate=" + escape(EndConfirmDate) + "";
    //设置检索条件
