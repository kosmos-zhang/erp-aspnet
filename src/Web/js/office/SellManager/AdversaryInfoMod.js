    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.data.length == 1) {
                ...
                fnGetDetail(data.data[0].CustNo); //获取报价单明细信息
            }
        }
    });
}
...
function fnInitPage(data) {
    $("#CustNo_txtCode").val(data.CustNo); //竞争对手编号                                 
    $("#CustType_ddlCodeType").val(data.CustType); //竞争对手类别(分类代码表定义，竞争对手类
    $("#CustClass").attr("title", data.CustClass); //竞争对手细分(往来单位分类表)  
    $("#CustClass").val(data.CodeName);
    $("#CustName").val(data.CustName); //竞争对手名称                                       
    $("#ShortNam").val(data.ShortNam); //竞争简称                                           
    $("#PYShort").val(data.PYShort); //拼音缩写                                            
    $("#AreaID_ddlCodeType").val(data.AreaID); //所在区域（分类代码表定义，区域代码）     
    $("#SetupDate").val(data.SetupDate); //成立时间                                          
    $("#ArtiPerson").val(data.ArtiPerson); //法人代表                                         
    $("#CompanyType").val(data.CompanyType); //单位性质（1事业，2企业，3社团，4自然人，5其他） 
    $("#StaffCount").val(data.StaffCount); //员工总数（个）                                   
    $("#SetupMoney").val(data.SetupMoney); //注册资本(万元)                                   
    $("#SetupAddress").val(data.SetupAddress); //注册地址                                       
    $("#website").val(data.website); //公司网址                                            
    $("#CapitalScale").val(data.CapitalScale); //资产规模(万元)                                 
    $("#SellArea").val(data.SellArea); //经营范围                                           
    $("#SaleroomY").val(data.SaleroomY); //年销售额(万元)                                    
    $("#ProfitY").val(data.ProfitY); //年利润额(万元)                                      
    $("#TaxCD").val(data.TaxCD); //税务登记号                                            
    $("#BusiNumber").val(data.BusiNumber); //营业执照号                                       
    $("#IsTax").val(data.IsTax); //一般纳税人（0否，1是）                                
    $("#Address").val(data.Address); //地址                                                
    $("#Post").val(data.Post); //邮编                                                   
    $("#ContactName").val(data.ContactName); //联系人                                          
    $("#Tel").val(data.Tel); //电话                                                    
    $("#Mobile").val(data.Mobile); //手机                                                 
    $("#email").val(data.email); //邮件                                                  
    $("#CustNote").val(data.CustNote); //竞争对手对手简介                                   
    $("#Product").val(data.Product); //主打产品                                            
    $("#Market").val(data.Market); //市场占有率                                           
    $("#SellMode").val(data.SellMode); //销售模式                                           
    $("#Project").val(data.Project); //竞争产品/方案                                       
    $("#Power").val(data.Power); //竞争能力                                              
    $("#Advantage").val(data.Advantage); //竞争优势                                          
    $("#disadvantage").val(data.disadvantage); //竞争劣势                                       
    $("#Policy").val(data.Policy); //应对策略                                             
    $("#Remark").val(data.Remark); //备注
    $("#UsedStatus").val(data.UsedStatus); //启用状态（0停用，1启用）                         
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.data != null) {
                ...
                AddSignRow();
                var rowID = i+1;
                $("#Dynamic" + rowID).val(item.Dynamic);
