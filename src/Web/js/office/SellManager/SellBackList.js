$(document).ready(function() {
    ...
    if (requestObj != null) {
        ...
        if (isSeacher == 'true') {
            ...
            $("#ddlReasonType").val(requestObj['Reason']); //退货原因
...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        ...
        url: '../../../Handler/Office/SellManager/SellBackLsit.ashx', //目标地址
        cache: false,
        data: strUrl,
        ...
        success: function(msg) {
            //数据获取完毕，填充页面据显示
            //数据列表
            $("#pageDataListSBackL tbody").find("tr.newrow").remove();
            $.each(msg.data, function(i, item) {
                if (item.ID != null && item.ID != "") {
                    var OrderNo = item.BackNo;
                    ...
                    $("<tr class='newrow'></tr>").append("<td height='22' align='center'>" + "<input id='chk" + i + "'  onclick = 'fnUnSelect(this)'  Title='" + item.BackNo + "' value='" + item.ID + "'   type='checkbox'/>" + "</td>" +
                    "<td height='22' align='center'><a href='#' title=\"" + item.BackNo + "\"  onclick=fnOrderInfo('" + item.ID + "','" + item.RefText + "')>" + OrderNo + "</a></td>" +
                    ...
                    "<td height='22' align='center'>" + item.BackDate + "</td>" +
                    "<td height='22' align='center'>" + FormatAfterDotNumber(item.TotalPrice, precisionLength) + "</td>" +
                    "<td height='22' align='center'>" + item.BillStatusText + "</td>" +
                    "<td height='22' align='center'>" + item.FlowInstanceText + "</td>").appendTo($("#pageDataListSBackL tbody"));
                }
            });
        ...
        complete: function() { hidePopup(); $("#pageDataList1_Pager").show(); Ifshow(document.form1.elements["Text2"].value); pageDataList1("pageDataListSBackL", "#E7E7E7", "#FFFFFF", "#cfc", "cfc"); } //接收数据完毕
    });
}
...
function fnUnSelect(obj) {
    ...
    if (!obj.checked) {
        ...
        //验证明细信息
        var signFrame = findObj("pageDataListSBackL", document);
...
function getSearchParams1() {
    ...
    var OfferDate = $("#OfferDate").val();      //计划回款金额
    var OfferDate1 = $("#OfferDate1").val();      //计划回款金额

    var Reason = $("#ddlReasonType").val(); //退货原因
    var FlowStatus = $("#FlowStatus").val();  //审批
    var BillStatus = $("#BillStatus").val();  //状态
    var ProjectID = $("#hiddProjectID").val();//所属项目ID

    $("#hiddExpOrderNo").val(OfferNo);
    $("#hiddExpTitle").val(Title);
    $("#hiddExpReasonType").val(Reason);
    ...
    var strUrl = 'orderNo=' + escape(OfferNo) + '&Title=' + escape(Title) + '&CustName=' + escape(CustName)
                + '&CustID=' + escape(CustID) + '&UserSeller=' + escape(UserSellerName) + '&Seller=' + escape(Seller)
                + '&FlowStatus=' + escape(FlowStatus) + '&OfferDate=' + escape(OfferDate)
                + '&OfferDate1=' + escape(OfferDate1) + '&Reason=' + escape(Reason)
...
//全选
function selectall() {
    $.each($("#pageDataListSBackL :checkbox"), function(i, obj) {
...
//新建
function fnNew() {
    window.location.href = 'SellBackAdd.aspx?' + getSearchParams() + '&islist=1&ModuleID=2031801';
}
...
function fnOrderInfo(retval, isRef) {
    window.location.href = 'SellBackMod.aspx?' + getSearchParams() + '&id=' + escape(retval) + "&islist=1&ModuleID=2031801&isRef=" + isRef;
}
...
function fnDel() {
    ...
    var msgText = "";
    var DetailID = '';
    var pageDataList1 = findObj("pageDataListSBackL", document);
...
function CheckInput() {
    ...
    if (FindDate != '' && FindDate1 != '') {
        if (CompareDate(FindDate, FindDate1) == 1) {
            isFlag = false;
            fieldText = fieldText + "退货日期|";
            msgText = msgText + "退货日期范围选择错误|";
        }
    }
//    //扩展属性
//    var EFIndex=document.getElementById("GetBillExAttrControl1_SelExtValue").value;//扩展属性select值
