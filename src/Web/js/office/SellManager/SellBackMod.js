...
function funSetCodeBar()
{
    try
    {
        ...
        var objBarCode = document.getElementById('GetGoods');
        hidCodeBarObj=='1'?objBarCode.style.display='':objBarCode.style.display='none';
    }catch(e){}
}
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.data.length == 1) {
                ...
                fnGetDetail(data.data[0].BackNo); //获取退货单明细信息

                fnIsRef(); //单据是否被其他单据引用
                fnStatus(data.data[0].BillStatus); //根据单据状态决定页面按钮操作
                fnGetBillStatus('5', '5', orderID); //获取审批状态，判断操作按钮
            }
        }
    });
}
...
function fnInitPage(data) {
    ...
    $("#CustTel").val(data.CustTel);         //客户电话
    $("#CustID").attr("title", data.CustID);         //客户ID（关联客户信息表）
    $("#FromBillID").val(data.SendNo);     //销售机会ID
    $("#FromBillID").attr("title", data.FromBillID);     //销售机会ID
    $("#BackCargoTheme").val(data.Title);          //报价主题
    ...
    $("#BackDate").val(data.BackDate);      //退货日期
    $("#SendAddress").val(data.SendAddress);   //发货地址
    $("#ReceiveAddress").val(data.ReceiveAddress); //收货地址
    $("#CountTotal").val(FormatAfterDotNumber(data.CountTotal,precisionLength));    //退货数量合计
    $("#NotPayTotal").val(FormatAfterDotNumber(data.NotPayTotal, precisionLength));   //抵应收账款

    $("#isSendText").val(data.isSendText);      //入库情况
    ...
    GetExtAttr("officedba.SellBack",data);
        ...
        success: function(data) {
            ...
            if (data.data != null) {
                $.each(data.data, function(i, item) {
                    ...
                    $("#ProductCount" + rowID).val(FormatAfterDotNumber(item.ProductCount, precisionLength)); //发货数量
                    ...
                    //$("#BackNumber" + rowID).val(FormatAfterDotNumber(item.BackNumber, 2)); //退货数量
                    ...
                    $("#ColorID" + rowID).val(item.ColorName); //规格
                    ...
                    $("#BDInNumber"+rowID).val(FormatAfterDotNumber(item.InNumber, precisionLength));//已入库数量
                    ...
                    if($("#txtIsMoreUnit").val() == "1")
                    {
                        ...
                        $("#BaseCountD" + rowID).val(FormatAfterDotNumber(item.BackNumber, precisionLength)); //基本数量
                        $("#BackNumber" + rowID).val(FormatAfterDotNumber(item.UsedUnitCount, precisionLength)); //数量
                    ...
                    else
                    {
                       $("#UnitID" + rowID).val(item.CodeName); //单位名称
                       $("#UnitID" + rowID).attr("title", item.UnitID); //单位ID  
                       $("#BackNumber" + rowID).val(FormatAfterDotNumber(item.BackNumber, precisionLength)); //数量
                       $("#UnitPrice" + rowID).val(FormatAfterDotNumber(item.UnitPrice, precisionLength)); //单价
                    }
                    ...
                    {   
                        ...
                        GetUnitGroupSelectEx(item.ProductID, 'SaleUnit', 'selectUnitID' + rowID, 'ChangeUnit('+rowID+')', "UnitID" + rowID, item.UsedUnitID,'fnDisabledOrNot('+rowID+')');
                    }
           ...
           success: function(msg){
                    ...
                    //填充物品ID，物品编号，物品名称,单位ID，单位名称，规格，
                        ...
                        {  
                            ...
                            $("#ColorID" + rowIndex).val(item.ColorName); //规格
...
//    if ($("#FromType").val() == 0) {
        ...
        if ($("#txtIsMoreUnit").val() == "1") 
        {
            ...
            newFitDesctd.className = "cell";
            newFitDesctd.innerHTML = "<input id='BaseUnitID" + rowID + "' disabled='disabled' type='text' class='tdinput' style=' width:90%;'/>"; //添加列内容
            m++;
            
            var newFitDesctd = newTR.insertCell(m); //添加列:基本数量(从物品档案带出不给修改)
            newFitDesctd.className = "cell";
            newFitDesctd.innerHTML = "<input id='BaseCountD" + rowID + "' disabled='disabled' type='text' class='tdinput' style=' width:90%;'/>"; //添加列内容
            m++;
        ...
        var newFitDesctd = newTR.insertCell(m); //添加列: 源单行号
        newFitDesctd.className = "cell";
        newFitDesctd.innerHTML = "<input id='BDInNumber" + rowID + "'  disabled='disabled'  type='text' class='tdinput' style=' width:94%;'/>"; //添加列内容
        m++;
...
//        $("#BackNumber" + rowID).val(FormatAfterDotNumber((item.ProductCount - item.BackCount), 2));
        //计量单位开启
        if ($("#txtIsMoreUnit").val() == "1") 
        {
            var newFitDesctd = newTR.insertCell(m); //添加列:退货数量
        ...
        //$("#UnitPrice" + rowID).val(FormatAfterDotNumber(item.UnitPrice, 2));
        //$("#hiddUnitPrice" + rowID).val(FormatAfterDotNumber(item.StandardCost, 2));
        m++;
               
        var newFitDesctd = newTR.insertCell(m); //添加列:含税价
        newFitDesctd.className = "cell";
        newFitDesctd.innerHTML = "<input id='TaxPrice" + rowID + "'  disabled='disabled'   type='text' class='tdinput' style=' width:94%;'/> <input type=\"hidden\" value='0.00' id='hiddTaxPrice" + rowID + "'/><input type=\"hidden\" value='0.00' id='hiddBaseTaxPrice" + rowID + "'/>"; //添加列内容
...
function getDropValue() {
    ...
    for (i = 1; i < signFrame.rows.length; i++) {
        if (signFrame.rows[i].style.display != "none") {
            ...
            var FromType = $("#FromType" + rowid).attr("title"); //源单类型（0无来源，1发货通知单）
            var FromBillID = $("#FromBillID" + rowid).attr("title");       //源单ID                      
            //var FromBillID = $("#FromBillID" + rowid).val();       //源单ID    
