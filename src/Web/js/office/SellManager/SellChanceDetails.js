$(document).ready(function(){
    ...
    {
        var search ="hidvalue="+escape(document.getElementById("hidvalue").value)+"&begintime="+document.getElementById("hidbeginTime").value+"&endtime="
        +document.getElementById("hidendTime").value+"&chanceType="+document.getElementById("hidChanceType").value
        +"&phase="+document.getElementById("hidPhase").value+"&state="+document.getElementById("hidState").value
        +"&flag="+document.getElementById("HidFlag").value;
        +"&deptid="+document.getElementById("HidDept").value+"&sellerid="+document.getElementById("HidSeller").value;;
            $.ajax({
           type: "POST",//用POST方式传输
           dataType:"json",//数据格式:JSON
           url:  '../../../Handler/Office/SellManager/SellChanceDetails.ashx',//目标地址
           ...
           success: function(msg){
                ...
                {  //var date=item.OperateDate;
                        $("<tr class='newrow'></tr>").append("<td height='22' align='center'>"+item.ChanceNo
                         +"</td>"+
                        ...
                        "<td height='22' align='center'>" + (parseFloat(item.IntendMoney)).toFixed($("#hidPoint").val()) + "</td>" +
                        "<td height='22' align='center'>"+item.Phase+"</td>"+
                        "<td height='22' align='center'>"+item.State+"</td>"+
                        "<td height='22' align='center'>"+item.FindDatecn+"</td>").appendTo($("#pageDataList1 tbody"));
                        }
                   });
                     //页码
