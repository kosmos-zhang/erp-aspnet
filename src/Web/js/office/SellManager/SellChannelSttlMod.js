...
function fnGetOrderInfo(orderID) {
    $.ajax({
        ...
        if (data.data.length == 1) {
                ...
                fnGetDetail(data.data[0].SttlNo); //获取报价单明细信息
                fnStatus(data.data[0].BillStatus); //根据单据状态决定页面按钮操作
                fnGetBillStatus('5', '6', orderID); //获取审批状态，判断操作按钮
            }
        }
    });
}
...
function fnInitPage(data) {
    ...
    $("#OfferNo").val(data.SttlNo);        //报价单编号

    $("#PayTypeUC_ddlCodeType").val(data.PayType);       //结算方式ID
    $("#MoneyTypeUC_ddlCodeType").val(data.MoneyType);       //结算方式ID
    $("#SttlDate").val(data.SttlDate); //结算日期
    $("#CountTotal").val(FormatAfterDotNumber(data.CountTotal,precisionLength));      //本次结算代销数量合计
    $("#TotalFee").val(FormatAfterDotNumber(data.TotalFee,precisionLength));        //本次结算代销金额合计
    $("#PushMoneyPercent").val(FormatAfterDotNumber(data.PushMoneyPercent,precisionLength)); //代销提成率
    $("#PushMoney").val(FormatAfterDotNumber(data.PushMoney, precisionLength));       //代销提成额
    if (data.HandFeeTotal != '') {
        $("#HandFeeTotal").val(FormatAfterDotNumber(data.HandFeeTotal, precisionLength));    //代销手续费
    }
    $("#SttlTotal").val(FormatAfterDotNumber(data.SttlTotal,precisionLength));       //应结金额合计
    $("#CustID").val(data.CustName);         //客户ID（关联客户信息表）
    $("#CustTel").val(data.Tel);         //客户ID（关联客户信息表）
    ...
    GetExtAttr("officedba.SellChannelSttl",data);
            ...
            if (data.data != null) {
                $.each(data.data, function(i, item) {
                    ...
                    if ($("#txtIsMoreUnit").val() == "1") 
                    {
                        ...
                        $("#BaseCountD" + rowID).val(FormatAfterDotNumber(item.SttlNumber, precisionLength));
                        m++;
                    ...
                    if($("#txtIsMoreUnit").val()=="1")
                    {
                        $("#SttlNumber" + rowID).val(FormatAfterDotNumber(item.UsedUnitCount, precisionLength));//本次结算数量
                        $("#UnitPrice" + rowID).val(FormatAfterDotNumber(item.UsedPrice, precisionLength));
                    }
                    else
                    {
                        $("#SttlNumber" + rowID).val(FormatAfterDotNumber(item.SttlNumber, precisionLength));//本次结算数量
...
function fnGetBillStatus(BillTypeFlag, BillTypeCode, BillID) {
    var Action = "Get";
    var UrlParam = "Action=Get&BillTypeFlag=" +escape(BillTypeFlag) + "&BillTypeCode=" +escape(BillTypeCode) + "&BillID=" +escape(BillID) + "";
    $.ajax({
        type: "POST",
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.ord != null) {
                ...
                $("#MoneyTypeUC_ddlCodeType").val(data.ord[0].PayType); //支付方式
...
function Fun_ConfirmOperate() {
    if (confirm("是否确认单据？")) {
        //var CodeType = ''; //单据编号产生的规则
        //var SttlNo = $("#OfferNo").val();        //退货单编号
