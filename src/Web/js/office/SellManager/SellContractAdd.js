...
GetExtAttr("officedba.SellContract",null);//扩展属性
    ...
    $("#orderNoUC_txtCode").css("width", "80px");
...
function fnTotalInfo() {
    ...
    for (i = 1; i < signFrame.rows.length; i++) {
        if (signFrame.rows[i].style.display != "none") {
            ...
            DiscountTotalDetail += ((100 - parseFloat(DiscountDetail)) / 100.0) * parseFloat(TaxPriceDetail) * parseFloat(pCountDetail);
        }
    }
    ...
    $("#DiscountTotal").val(FormatAfterDotNumber(((TotalFee * (100 - Discount) / 100.0) + parseFloat(DiscountTotalDetail)), precisionLength));
    $("#RealTotal").val(FormatAfterDotNumber((TotalFee * parseFloat(Discount) / 100.0), precisionLength));
}
...
function Fun_ConfirmOperate() {
    if (confirm("是否确认单据？")) {
        ...
        var ContractNo = $("#orderNoUC_txtCode").val();        //退货单编号
        $.ajax({
            type: "POST",
            url: "../../../Handler/Office/SellManager/SellContractAdd.ashx",
...
function Fun_FillParent_Content(ID, ProdNo, ProductName, StandardSell, UnitID, CodeName, TaxRate, SellTax, Discount, Specification,CodeTypeName,TypeID,StandardBuy,TaxBuy,InTaxRate,StandardCost,GroupUnitNo,SaleUnitID,SaleUnitName,InUnitID,InUnitName,StockUnitID,StockUnitName,MakeUnitID,MakeUnitName,IsBatchNo) {
   ...
   var rowIndex=popTechObj.InputObj;
...
function fnGetInfo() {
    var strInfo = '';
    var CodeType = $("#orderNoUC_ddlCodeRule").val(); //报价单编号产生的规则
    var ContractNo = $("#orderNoUC_txtCode").val();  //合同编号       
...
//    if ($("#txtIsMoreUnit").val() == "1") {
//        SetFunNo = "SetEventFocus('ProductID','TD_Text_UsedUnitCount_'," + rowID + ",false);";
//    }
...
function CheckInput() {
    ...
    var isFlag = true;
    var msgText = "";
    var CodeType = $.trim($("#orderNoUC_ddlCodeRule").val()); //合同编号产生的规则
    var OfferNo = $.trim($("#orderNoUC_txtCode").val()); //合同编号                                                        
    ...
    if (CodeType == "") {
        if (OfferNo == "") {
            ...
            msgText = msgText + "请输入合同编号|";
        }
        else {
            if (!CodeCheck(OfferNo)) {
                isFlag = false;
                fieldText = fieldText + "合同编号|";
    ...
    for (i = 1; i < signFrame.rows.length; i++) {
        if (signFrame.rows[i].style.display != "none") {
            ...
            if (TaxRate == "") {
                ...
                if (!IsNumeric(TaxRate, 14, precisionLength)) {
                    isFlag = false;
                    fieldText = fieldText + "合同明细（行号：" + i + "）|";
            ...
            if (SendTime == "") {
                ...
                if (!IsNumeric(SendTime, 14, precisionLength)) {
                    isFlag = false;
                    fieldText = fieldText + "合同明细（行号：" + i + "）|";
