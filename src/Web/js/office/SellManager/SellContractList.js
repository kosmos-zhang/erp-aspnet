...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        ...
        url: '../../../Handler/Office/SellManager/SellContractList.ashx', //目标地址
        cache: false,
        data: strUrl,
        ...
        success: function(msg) {
            ...
            if (item.ID != null && item.ID != "") {
                ...
                $("<tr class='newrow'></tr>").append("<td height='22' align='center'>" + "<input id='chk" + i + "'   onclick = 'fnUnSelect(this)'  Title='" + item.ContractNo + "'    value='" + item.ID + "'   type='checkbox'/>" + "</td>" +
                    "<td height='22' align='center'><a href='#' title=\"" + item.ContractNo + "\" onclick=fnOrderInfo('" + item.ID + "','" + item.RefText + "')>" + ContractNo + "</a></td>" +
...
//新建
function fnNew() {
    window.location.href = 'SellContractAdd.aspx?' + getSearchParams() + '&islist=1&ModuleID=2031401';
}
...
function fnOrderInfo(retval, isRef) {
    window.location.href = 'SellContractMod.aspx?' + getSearchParams() + '&id=' + escape(retval) + "&islist=1&ModuleID=2031401&isRef=" + isRef;
}
...
function CheckInput() {
    ...
    var SendNo = $.trim($("#orderNo").val()); //报价单编号    
