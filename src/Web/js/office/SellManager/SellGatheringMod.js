...
function fnGetOrderInfo(orderID) {
    $.ajax({
        type: "POST",
        url: "../../../Handler/Office/SellManager/SellGatheringList.ashx",
...
function fnInitPage(data) {
    $("#OfferNo").val(data.GatheringNo);        //报价单编号
    $("#Title").val(data.Title);         //主题

    $("#CustID").val(data.CustName);         //客户ID（关联客户信息表）
    $("#CustID").attr("title", data.CustID);         //客户ID（关联客户信息表）
    $("#CustType").val(data.TypeName);         //客户电话
    $("#CustTel").val(data.Tel);         //客户类型
    $("#FromType").val(data.FromType);       //源单类型（0无来源，1发货通知单，2销售订单）
    $("#FromBillID").val(data.BillNo);     //销售机会ID
    ...
    $("#PlanGatherDate").val(data.PlanGatherDate); //计划回款日期
    $("#PlanPrice").val(FormatAfterDotNumber(data.PlanPrice,precisionLength));      //计划回款金额
    $("#GatheringTime").val($.trim(data.GatheringTime));  //期次
    $("#FactPrice").val(FormatAfterDotNumber(data.FactPrice,precisionLength));      //实际回款金额
    $("#FactGatherDate").val(data.FactGatherDate); //实际回款日期
    $("#UserSeller").val(data.EmployeeName);         //业务员(对应员工表ID)
    $("#hiddSeller").val(data.Seller);         //业务员(对应员工表ID)
    $("#hiddDeptID").val(data.SellDeptId);     //部门(对部门表ID)
    $("#DeptId").val(data.DeptName);     //部门(对部门表ID)
    $("#LinkBillNo").val(data.LinkBillNo);     //回款相关单据号
    $("#State").val(data.State);          //状态(1已回款2未回款 3部分回款)
    ...
    GetExtAttr("officedba.SellGathering",data);
}
...
//保存数据
function InsertSellGatheringData() {
    if (!CheckInput()) {
        return;
    }
    var Title = $.trim($("#Title").val());         //主题    
    var CodeType = ''; //回款计划编号产生的规则
    var GatheringNo = $("#OfferNo").val(); //回款计划编号                                  
    ...
    $.ajax({
        ...
        '&Remark=' + escape(Remark) + '&action=' + escape(str)+GetExtAttrValue(),
        dataType: 'json', //返回json格式数据
        cache: false,
        beforeSend: function() {
            AddPop();
        ...
        success: function(data) {
            if (data.sta == 1) {
                fnGetOrderInfo($("#hiddOrderID").val());
            }
