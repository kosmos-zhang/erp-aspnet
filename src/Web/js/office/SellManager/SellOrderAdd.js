...
fnGetExtAttr();
    ...
    $("#SellOrderUC_ddlCodeRule").css("width", "80px");
    $("#SellOrderUC_txtCode").css("width", "80px");
...
function fnAddFeeRow() {
    ...
    newFitNametd.innerHTML = "<input id='FeeTotal" + rowID + "' onblur=\"fnFee()\"  style=' width:85%;' type='text'  maxlength='12' class='tdinput' />"; //添加列内容

    var newFitDesctd = newTR.insertCell(3); //添加列:备注
    newFitDesctd.className = "cell";
    newFitDesctd.innerHTML = "<input id='FeeRemark" + rowID + "' type='text' class='tdinput' style=' width:85%;'  maxlength='100' />"; //添加列内容
            ...
            if (data.conList != null) {
                $.each(data.conList, function(i, item) {
                    ...
                    if($("#txtIsMoreUnit").val() == "1")
                    {
                        ...
                        $("#BaseProductCount" + rowID).val(item.ProductCount); //基本数量
...
function Fun_ConfirmOperate() {
    if (confirm("是否确认单据？")) {
        var CodeType = $("#SellOrderUC_ddlCodeRule").val(); //报价单编号产生的规则
        var OrderNo = $("#SellOrderUC_txtCode").val();        //退货单编号
        var ID=$("#hiddOrderID").val();
        
        $.ajax({
            type: "POST",
            url: "../../../Handler/Office/SellManager/sellOrderAdd.ashx",
            data: 'CodeType=' + escape(CodeType) + '&OrderNo=' + escape(OrderNo) + '&ID='+escape(ID)+getMainParamValue()+'&action=confirm',
            dataType: 'json', //返回json格式数据
            cache: false,
            beforeSend: function() {
                AddPop();
...
function Fun_UnConfirmOperate() {
    if (confirm("是否取消确认？")) {
        ...
        $.ajax({
            type: "POST",
            url: "../../../Handler/Office/SellManager/sellOrderAdd.ashx",
            data: 'CodeType=' + escape(CodeType) + '&OrderNo=' + escape(OrderNo) +'&ID='+escape(ID)+getMainParamValue()+ '&action=' + acction,
            dataType: 'json', //返回json格式数据
            cache: false,
            beforeSend: function() {
                AddPop();
...
function Fun_FillParent_Content(ID, ProdNo, ProductName, StandardSell, UnitID, CodeName, TaxRate, SellTax, Discount, Specification,CodeTypeName,TypeID,StandardBuy,TaxBuy,InTaxRate,StandardCost,GroupUnitNo,SaleUnitID,SaleUnitName,InUnitID,InUnitName,StockUnitID,StockUnitName,MakeUnitID,MakeUnitName,IsBatchNo,ColorName) {
    ...
    if(!IsExist2(ProdNo,rowIndex))//如果是重复的物品编号不允许添加了
    {
        ...
        //$("#UnitPrice" + rowIndex).val(FormatAfterDotNumber(StandardSell, precisionLength)); //单价（去税售价）
...
function InsertSellOfferData() {
    ...
    var CodeType = $("#SellOrderUC_ddlCodeRule").val(); //订单编号产生的规则
    var OrderNo = $("#SellOrderUC_txtCode").val(); //订单编号 
    var BillStatus = '1';
    var str = 'insert';
    $.ajax({
        type: "POST",
        ...
        data: 'CodeType=' + escape(CodeType) + '&OrderNo=' + escape(OrderNo) + '&BillStatus=' + escape(BillStatus) +
             getMainParamValue()+
             '&strfitinfo=' + escape(strfitinfo) + '&strFeeInfo=' + escape(strFeeInfo) + '&action=' + escape(str)+GetExtAttrValue(),
        dataType: 'json', //返回json格式数据
        cache: false,
        beforeSend: function() {
            AddPop();
        ...
        success: function(data) {
            if (data.sta == 1) {
                if (data.sta == 1) {
                    ...
                    $("#btn_update").css("display", "inline");
                    $("#SellOrderUC_txtCode").val(data.no);
                    $("#SellOrderUC_txtCode").attr("disabled", "true");
                    $("#SellOrderUC_ddlCodeRule").css("display", "none");
                    $("#SellOrderUC_txtCode").css("width", "95%");
                    GetFlowButton_DisplayControl();
...
function CheckInput() {
    ...
    var msgText = "";
    var CodeType = $("#SellOrderUC_ddlCodeRule").val(); //订单编号产生的规则
    var OrderNo =  $.trim($("#SellOrderUC_txtCode").val()); //订单编号                                                                 
...
//    }
    ...
    for (i = 1; i < signFrame.rows.length; i++) {
        if (signFrame.rows[i].style.display != "none") {
            ...
            if (ProductCount == "" || ProductCount==zeroP) {
                isFlag = false;
                fieldText = fieldText + "订单明细（行号：" + i + "）|";
...
function getZeroPiontStyle()
{
    var zeroStr="0.";//小数位数长度的0
...
function fnPrintOrder() {
    var OrderNo = $.trim($("#SellOrderUC_txtCode").val());
