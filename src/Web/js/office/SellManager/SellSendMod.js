    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.data.length == 1) {
                ...
                fnGetDetail(data.data[0].SendNo); //获取报价单明细信息

                fnIsRef(); //单据是否被其他单据引用
                fnStatus(data.data[0].BillStatus); //根据单据状态决定页面按钮操作
                fnGetBillStatus('5', '4', orderID); //获取审批状态，判断操作按钮
            }
        }
    });
}
...
function fnInitPage(data) {
    ...
    $("#FromBillID").val(data.OrderNo);     //销售机会ID
    ...
    $("#SendAddr").val(data.SendAddr);      //发货地址
    $("#hiddSender").val(data.Sender);        //发货人ID（对应员工表ID）
    $("#UserSender").val(data.SenderName);        //发货人ID（对应员工表ID）
    $("#ReceiveAddr").val(data.ReceiveAddr);   //收货地址
    $("#Receiver").val(data.Receiver);      //收货人姓名
    $("#Tel").val(data.Tel);           //收货人电话
    $("#Modile").val(data.Modile);        //收货人移动电话
    $("#Post").val(data.Post);          //收货人邮编
    $("#SendDate").val(data.IntendSendDate); //预计发货时间
    $("#Transporter").val(data.TransporterName); //运输商
    $("#Transporter").attr("title", data.Transporter);   //运输商
    $("#TransportFee").val(FormatAfterDotNumber(data.TransportFee, precisionLength));  //运费合计
    $("#TransPayType_ddlCodeType").val(data.TransPayType);  //运费结算方式ID
    ...
    $("#hiddIsOpenbill").val(data.isOpenbill);//是否已开票：1已开票，0未开票
    
    GetExtAttr("officedba.SellSend",data);
    if(data.BillStatus=="2" && data.isOpenbill=="0")
    {
        ...
        success: function(data) {
            ...
            if (data.data != null) {
                $.each(data.data, function(i, item) {
                    ...
                    newFitNametd.innerHTML = "<input id='ProductName" + rowID + "'   disabled='disabled'  readonly='readonly'  style=' width:80%; ' type='text'  class='tdinput' />"; //添加列内容
                    m++;

                    var newFitDesctd = newTR.insertCell(m); //添加列:规格
                    newFitDesctd.className = "cell";
                    newFitDesctd.innerHTML = "<input  id='Specification" + rowID + "'   disabled='disabled'  style=' width:80%;'readonly='readonly' type='text'  class='tdinput' />"; //添加列内容
                    m++;

                    var newFitDesctd = newTR.insertCell(m); //添加列:颜色
                    newFitDesctd.className = "cell";
                    newFitDesctd.innerHTML = "<input  id='ColorID" + rowID + "'   disabled='disabled'  style=' width:80%;'readonly='readonly' type='text'  class='tdinput' />"; //添加列内容
                    m++;
                    ...
                    newFitDesctd.className = "cell";
                    newFitDesctd.innerHTML = "<input  id='OrderCount" + rowID + "'   disabled='disabled'  readonly='readonly' type='text'  class='tdinput'  style=' width:80%;'/>"; //添加列内容
                    m++;

                    var newFitDesctd = newTR.insertCell(m); //添加列:已执行数量
                    newFitDesctd.className = "cell";
                    newFitDesctd.innerHTML = "<input id='transactCount" + rowID + "'   disabled='disabled'  readonly='readonly' type='text'   class='tdinput'  style=' width:80%;'/>"; //添加列内容
                    m++;
                    ...
                    newFitDesctd.className = "cell";
                    newFitDesctd.innerHTML = "<input id='FromBillID" + rowID + "'   disabled='disabled'  readonly='readonly' type='text' class='tdinput' style=' width:95%;'/>"; //添加列内容
                    m++;

                    var newFitDesctd = newTR.insertCell(m); //添加列: 源单行号
                    newFitDesctd.className = "cell";
                    newFitDesctd.innerHTML = "<input id='FromLineNo" + rowID + "'   disabled='disabled'  readonly='readonly' type='text' class='tdinput' style=' width:80%;'/>"; //添加列内容
                    m++;

                    var newFitDesctd = newTR.insertCell(m); //添加列: 已退货数量
                    newFitDesctd.className = "cell";
                    newFitDesctd.innerHTML = "<input id='BackCount" + rowID + "'   disabled='disabled'  readonly='readonly' type='text' class='tdinput' style=' width:80%;'/>"; //添加列内容
                    m++;

                    var newFitDesctd = newTR.insertCell(m); //添加列: 已出库数量
                    newFitDesctd.className = "cell";
                    newFitDesctd.innerHTML = "<input id='OutCount" + rowID + "'   disabled='disabled'  readonly='readonly' type='text' class='tdinput' style=' width:80%;'/>"; //添加列内容
                    m++;

                    var newFitDesctd = newTR.insertCell(m); //添加列: 已结算数量
                    newFitDesctd.className = "cell";
                    newFitDesctd.innerHTML = "<input id='SttlCount" + rowID + "'   disabled='disabled'  readonly='readonly' type='text' class='tdinput' style=' width:80%;'/>"; //添加列内容
                    m++;

                    $("#BackCount" + rowID).val(FormatAfterDotNumber(item.BackCount, precisionLength)); //已退货数量
                    $("#OutCount" + rowID).val(FormatAfterDotNumber(item.OutCount, precisionLength)); //已出库数量
                    ...
                    if (item.transactCount != "") {
                        $("#transactCount" + rowID).val(FormatAfterDotNumber(item.transactCount, precisionLength)); //已执行数量
                    }
//                    else
//                    {
//                        $("#transactCount" + rowID).val(FormatAfterDotNumber(item.transactCount, precisionLength)); //已执行数量
...
function fnBack() {
    ...
    if (requestObj != null) {
        ...
        if (flag != null && typeof (flag) != "undefined") {
            ...
            {
                var requestobj = GetRequestToHidden();
                requestobj = requestobj.replace('ModuleID=2031601', 'ModuleID='+ListModuleID);
                document.getElementById("HiddenURLParams").value = requestobj;
                var URLParams = document.getElementById("HiddenURLParams").value;
                window.location.href = 'SellSendDetailsList.aspx'+URLParams;//跳转到销售发货明细列表
            }
            else
            {
                var URLParams = document.getElementById("HiddenURLParams").value;
...
function fnSetDetailData(data) {
    $.each(data, function(i, item) {
        ...
        if ($("#txtIsMoreUnit").val() == "1") 
        {
            ...
            newFitDesctd.className = "cell";
            newFitDesctd.innerHTML = "<input id='BaseCountD" + rowID + "' disabled='disabled' type='text' class='tdinput' style=' width:80%;'/>"; //添加列内容
            $("#BaseCountD" + rowID).val(item.pCount);
            m++;
        ...
        var newFitDesctd = newTR.insertCell(m); //添加列:包装要求
        newFitDesctd.className = "cell";
        newFitDesctd.innerHTML = "<select id='Package" + rowID + "' disabled='disabled'  style=' width:80%;'></select>"; //添加列内容
        fnGetPackage("Package" + rowID); //添加包装要求选择项的值
        ...
        var newFitDesctd = newTR.insertCell(m); //添加列:含税价
        newFitDesctd.className = "cell";
        newFitDesctd.innerHTML = "<input id='TaxPrice" + rowID + "' disabled='disabled' type='text' class='tdinput' style=' width:80%;'/> <input type=\"hidden\" value='0.00' id='hiddTaxPrice" + rowID + "'/><input type=\"hidden\" value='0.00' id='hiddBaseUnitPrice" + rowID + "'/>"; //添加列内容
...
function fnDisabledOrNot(rowID)
{
    ...
    //document.getElementById("#selectUnitID"+rowID).disabled=true;
...
function fnConfirmOperate() {
        ...
        //var CodeType = ''; //发货单编号产生的规则
        //var SendNo = $("#OfferNo").val();        //退货单编号
...
function fnGetInfo() {
    var strInfo = '';
    var CodeType = ''; //发货单编号产生的规则
    var SendNo = $("#OfferNo").val(); //发货单编号    
    var Title = $.trim($("#Title").val());         //主题
    var FromType ='1';      //源单类型（0无源单，1销售发货单，2销售合同）
    var FromBillID = $("#FromBillID").attr("title"); //来源单据编号                       
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.data.length == 1) {
                ...
                /*----------判断是来自明细列表还是来自明细列表Start ：若是来自明细列表则带上明细列表的标志和Moduleid，为返回设置参数------------*/
                var strP="";
                var requestObj = GetRequest(); //参数列表
                if (requestObj != null) {
                    var flag = requestObj['id']; //是否从列表页面进入
                    if (flag != null && typeof (flag) != "undefined") {
                        var intFromType = requestObj['intFromType']; //是否从列表页面进入           
                        var ListModuleID = requestObj['ListModuleID']; //来源页MODULEID 
                        if (intFromType != null && typeof (intFromType) != "undefined")
                        {
                            strP="&intFromType="+intFromType;
                        }
                        if (ListModuleID != null && typeof (ListModuleID) != "undefined")
                        {
                            strP+="&ListModuleID="+ListModuleID;
                        }
                    }
                }
                /*----------判断是来自明细列表还是来自明细列表Start------------*/
                window.location.href="../FinanceManager/Billing_Add.aspx?aid="+escape(data.data[0].ID)+"&ParamLoc="+ParamLoc+"&ModuleID="+billingmoduleid+"&islist=1"+strP;
            }
        }
    });
