$(document).ready(function(){
    ...
    function TurnToPage(pageIndex)
    {
        ...
        +document.getElementById("HidTimeType").value+"&storageid="+document.getElementById("HidStorageID").value+"&timestr="+escape(document.getElementById("hidvalue").value)+"&ByTimeType="+escape(document.getElementById("HiddenField1").value);

         $.ajax({
             type: "POST", //用POST方式传输
             dataType: "json", //数据格式:JSON
             url: '../../../Handler/OperatingModel/StorageManager/StorageInSetUpDetails.ashx', //目标地址
             ...
             success: function(msg) {
                 ...
                 $.each(msg.data, function(i, item) {
                     ...
                     if (item.id != null && item.id != "") {  //var date=item.OperateDate;

                         if (document.getElementById("hidDisplayPrice").value == "1") {
                        ...
                        "<td height='22' align='center'>" + item.ModifiedDateCn + "</td>").appendTo($("#pageDataList1 tbody"));
                             document.getElementById("ThshowPrice").style.display = "block";
                         }
                         else {
