...
function GetRequest()
{
    ...
    function TurnToPage(pageIndex)
    {
           ...
           url:  '../../../Handler/Office/SubStoreManager/StorageList.ashx',//目标地址
           cache:false,
           ...
           success: function(msg){
                    ...
                    $.each(msg.data,function(i,item){
                        ...
                        if (StorageNo != null) {
                        if (StorageNo.length > 8) {
                            StorageNo = StorageNo.substring(0, 8) + '...';
                            }
                        }
                        var StorageName = item.StorageName;
                        if (StorageName != null) {
                        if (StorageName.length > 6) {
                            StorageName = StorageName.substring(0, 6) + '...';
                            }
                        }
                        ...
                        "<td height='22' align='center'><span title=\"" + item.StorageNo + "\">"+StorageNo+"</a></td>"+
                        "<td height='22' align='center'><span title=\"" + item.StorageName + "\">"+StorageName+"</a></td>"+
                        "<td height='22' align='center'><span title=\"" + item.ProductNo + "\">"+ProductNo+"</a></td>"+
                        "<td height='22' align='center'>"+item.ProductName+"</a></td>"+
                        "<td height='22' align='center'>"+ item.TypeIDName +"</a></td>"+
                        "<td height='22' align='center'>"+ item.Specification +"</a></td>"+
                        "<td height='22' align='center'>"+ item.UnitName +"</a></td>"+
                        "<td height='22' align='center'>"+FormatAfterDotNumber( item.ProductCount,selPoint) +"</a></td>"+
                        "<td height='22' align='center'>"+ item.BatchNo +"</a></td>"
...
//检索条件
    ...
    var StorageID = document.getElementById("drpStorageName").value;//仓库ID
    if(StorageID == 0)
    {
        StorageID="";
    }
